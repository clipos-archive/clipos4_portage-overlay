...
inherit eutils autotools flag-o-matic verictl2
...
src_install() {
	...
	insopts -m 644 ; insinto "${CPREFIX}"/etc/elinks
	...
	insinto "${CPREFIX:-/usr}"/share/doc/${PF}/contrib/lua ; doins contrib/lua/{*.lua,elinks-remote}
	insinto "${CPREFIX:-/usr}"/share/doc/${PF}/contrib/conv ; doins contrib/conv/*.*
	insinto "${CPREFIX:-/usr}"/share/doc/${PF}/contrib/guile ; doins contrib/guile/*.scm

	# Remove some conflicting files on OSX.  The files provided by OSX 10.4
	# are more or less the same.  -- Fabian Groffen (2005-06-30)
	rm -f "${D}${CPREFIX:-/usr}"/share/locale/locale.alias "${D}${CPREFIX:-/usr}"/lib/charset.alias
}

pkg_postinst() {
	einfo "This ebuild provides a default config for ELinks."
	einfo "Please check ${CPREFIX}/etc/elinks/elinks.conf"
	einfo
	...
	einfo "using ${CPREFIX:-/usr}/share/doc/${PF}/contrib/conv/conf-links2elinks.pl"
	einfo
	einfo "Please have a look at ${CPREFIX}/etc/elinks/keybind-full.sample and"
	einfo "${CPREFIX}/etc/elinks/keybind.conf.sample for some bindings examples."
	einfo
...
pkg_predeb() {
	doverictld2  "${CPREFIX:-/usr}"/bin/elinks e - - - c ccsd
}
