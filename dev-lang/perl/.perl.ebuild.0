...
IUSE="berkdb debug doc gdbm ithreads clip clip-core clip-devstation"
...
"
PDEPEND="!clip? ( >=app-admin/perl-cleaner-2.5 )"
...
#Begin CLIP Patches
	epatch "${FILESDIR}"/${PN}-5.16.3-do-not-leak-temp-utf8-copy-of-namepv.patch
	epatch "${FILESDIR}"/${PN}-5.16.3-VMS.patch
	epatch "${FILESDIR}"/${PN}-5.16.3-remove-duplicate-env-variables.patch 
#End

	if use clip-devstation; then
		epatch "${FILESDIR}"/${PN}-5.16.3-clip-devstation-mod_path.patch
	fi
	if use clip; then 
		epatch "${FILESDIR}"/${PN}-5.16.3-clip-mayexec.patch
		epatch "${FILESDIR}"/${PN}-5.16.3-clip-mod_path.patch
	fi
}
...
src_install()	{ 
	...
	if use clip-core; then
		...
		cp "${D}/usr/bin/perl" "${D}/mounts/admin_root/usr/bin/perl" \
			|| die "perl copy failed"
	fi
}
