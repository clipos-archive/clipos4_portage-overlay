...
inherit autotools eutils prefix multilib-minimal verictl2
...
~hppa-hpux ~ia64-hpux ~x86-interix ~amd64-linux ~arm-linux ~x86-linux ~ppc-macos
...
IUSE+=" elibc_Winnt"
IUSE+=" clip clip-core clip-rm clip-devstation rm-deps"
...
RDEPEND="ldap? ( >=net-nds/openldap-2.4.38-r1[${MULTILIB_USEDEP}] )
	...
	kerberos? ( 
		!clip? ( >=virtual/krb5-0-r1[${MULTILIB_USEDEP}] )
		clip? ( app-crypt/mit-krb5[${MULTILIB_USEDEP}] )
	)
	metalink? ( >=media-libs/libmetalink-0.1.1[${MULTILIB_USEDEP}] )
	rtmp? ( >=media-video/rtmpdump-2.4_p20131018[${MULTILIB_USEDEP}] )
	ssh? ( >=net-libs/libssh2-1.4.3[static-libs?,${MULTILIB_USEDEP}] )
	!rm-deps? ( >=sys-libs/zlib-1.2.8-r1[${MULTILIB_USEDEP}] )
	abi_x86_32? (
...
src_prepare() {
	...
	# clip fix: undefined reference to "hugehelp"
	#epatch "${FILESDIR}/${PN}-7.36.0-mkhelp-generate-code-for-disable-manual.patch"
	# Regenerate an empty tool_hugehelp.c file (manual disabled):
	#rm "${WORKDIR}/${P}/src/tool_hugehelp.c"
...
multilib_src_configure() {
	...
	if ! use clip-core; then
		myconf+=( --with-ca-bundle="${EPREFIX}${CPREFIX}"/etc/ssl/certs/ca-certificates.crt )
	fi
	if use ssl ; then
		...
		if use curl_ssl_openssl; then
			...
			myconf+=( --with-ssl --with-ca-path="${EPREFIX}${CPREFIX}"/etc/ssl/certs )
		fi
...
multilib_src_install_all() {
	...
	rm -rf "${ED}${CPREFIX}"/etc/

	if use clip-core; then
		dodir "/mounts/admin_root/usr/bin"
		cp -a "${ED}/${CPREFIX:-/usr}/bin/curl" \
			"${ED}/mounts/admin_root/usr/bin/curl" || die
	fi
	...
	if use clip-devstation; then
		echo '#warning Deprecated header' \
			> "${ED}${CPREFIX:-/usr}/include/curl/types.h" \
			|| die ""
	fi
}
...
pkg_predeb() {
	use clip-rm && \
		doverictld2  ${CPREFIX:-/usr}/bin/curl e - - - c 
	if use clip-core; then
		VERIEXEC_CTX=504 doverictld2 /mounts/admin_root/usr/bin/curl e - - - c
	fi
}
