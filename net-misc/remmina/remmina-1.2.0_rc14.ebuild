...
inherit versionator gnome2-utils cmake-utils verictl2
...
DESCRIPTION_FR="Client de déport d'affichage RDP, VNC et SSH"
CATEGORY_FR="Réseau"
...
IUSE="ayatana avahi crypt debug freerdp gnome-keyring nls ssh telepathy vte
      clip clip-devstation clip-deps"
...
RDEPEND="
	...
	!clip-deps? ( >=dev-libs/glib-2.31.18:2 )
...
src_prepare() {
	if use clip; then
		epatch "${FILESDIR}/clip-rdp-default-shared-folder.patch"
	fi
}
...
pkg_predeb() {
	doverictld2  "${CPREFIX:-/usr}/bin/remmina" e - - - c
}
