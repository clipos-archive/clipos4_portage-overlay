...
inherit flag-o-matic eutils toolchain-funcs verictl2
if [[ ${PV} == "99999999" ]] ; then
...
LICENSE="BSD"
SLOT="0"
IUSE="doc idn ipv6 SECURITY_HAZARD ssl static clip clip-devstation"
...
src_prepare() {
	...
	use clip && epatch "${FILESDIR}"/${PN}-20101006-clip-pingsock.patch
...
src_install() {
	...
	if use clip; then
		# setgid 322 (ping) to enable access to ping sockets
		local sgidbins="/bin/ping"
		use ipv6 && sgidbins="${sgidbins} /bin/ping6 /usr/sbin/traceroute6"
		for exe in ${sgidbins} ; do
			fowners 0:322 "${exe}"
			fperms 2711 "${exe}"
		done
	else
...
pkg_predeb() {
	doverictld2  ${CPREFIX}/bin/ping e - - - c 
	doverictld2  ${CPREFIX}/sbin/arping e - - - c 
	doverictld2  ${CPREFIX:-/usr}/sbin/tracepath e - - - c 
}
