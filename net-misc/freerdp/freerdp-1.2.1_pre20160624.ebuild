...
inherit cmake-utils vcs-snapshot verictl2 eutils
...
IUSE="alsa +client cpu_flags_x86_sse2 cups debug doc ffmpeg gstreamer jpeg
    ...
    clip-deps core-deps clip clip-rm
    "
...
src_prepare() {
	#apply the patch to add the dynamic channel dvcsocket
	if use clip; then 
		if use clip-rm; then 
	 		#cp -r "${FILESDIR}/dvcsocket" "${S}/channels/dvcsocket"
			#epatch "${FILESDIR}/${MY_P}-drdynvc.patch"
			#epatch "${FILESDIR}/${MY_P}-DYNVC_CLOSE.patch" 
			epatch "${FILESDIR}/${PN}-clip-custom-rpath.patch"
		fi
	fi
}
...
pkg_predeb() {
	use X && \
		doverictld2  "${CPREFIX:-/usr}/bin/xfreerdp" e - - - c
}
