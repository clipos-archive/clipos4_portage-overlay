...
# $Header: /var/cvsroot/gentoo-x86/net-fs/samba/samba-3.6.24.ebuild,v 1.11 2014/12/26 09:18:35 ago Exp $

EAPI=5

inherit pam versionator multilib multilib-minimal eutils flag-o-matic systemd verictl2
...
DEPEND="dev-libs/popt
	...
	!clip-deps? ( >=virtual/libiconv-0-r1[${MULTILIB_USEDEP}] )
	ads? ( >=virtual/krb5-0-r1[${MULTILIB_USEDEP}] sys-libs/e2fsprogs-libs
		client? ( sys-apps/keyutils
			!clip? ( kernel_linux? ( net-fs/cifs-utils[ads] ) ) ) )
	avahi? ( net-dns/avahi[dbus] )
	caps? ( >=sys-libs/libcap-2.22-r2[${MULTILIB_USEDEP}] )
	client? ( !net-fs/mount-cifs
		>=dev-libs/iniparser-3.1-r1[${MULTILIB_USEDEP}]
		!clip? ( kernel_linux? ( net-fs/cifs-utils ) ) )
...
src_prepare() {
	...
	if use clip ; then
		epatch "${FILESDIR}/samba-3.6.24-clip-smbpoll_disable_conf_file.patch"
		epatch "${FILESDIR}/samba-3.6.24-clip-only-port-139-does-netbios.patch"
	fi
...
multilib_src_configure() {
	...
	econf "${myconf[@]}" \
		--with-piddir="${EPREFIX}${statedir}"/run/samba \
		--sysconfdir="${EPREFIX}${CPREFIX}"/etc/samba \
		...
		--with-fhs \
		--with-privatedir="${EPREFIX}${statedir}"/lib/samba/private \
		--with-rootsbindir="${EPREFIX}${statedir}"/cache/samba \
		--with-lockdir="${EPREFIX}${statedir}"/cache/samba \
		--with-swatdir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/swat \
		--with-configdir="${EPREFIX}${CPREFIX}"/etc/samba \
		--with-logfilebase="${EPREFIX}${statedir}"/log/samba \
...
multilib_src_install_all() {
	...
	if use server ; then
		...
		diropts -m0700
		keepdir ${statedir}/lib/samba/private

		diropts -m1777
		keepdir ${statedir}/spool/samba

		diropts -m0755
		keepdir ${statedir}/{cache,log}/samba
		keepdir ${statedir}/lib/samba/{netlogon,profiles}
		keepdir ${statedir}/lib/samba/printers/{W32X86,WIN40,W32ALPHA,W32MIPS,W32PPC,X64,IA64,COLOR}
		...
		if use swat ; then
			...
			script/installswat.sh "${ED}" "${EROOT}${CPREFIX:-/usr}/share/doc/${PF}/swat" "${S}"
		fi
	...
	if use cups ; then
		...
		use clip && cupsbin="/usr/lib/cups"
		dosym /usr/bin/smbspool ${cupsbin}/backend/smb
	fi
	...
	rmdir --ignore-fail-on-non-empty \
		"${ED}${CPREFIX:-/usr}/$(get_libdir)/samba" \
		"${ED}${CPREFIX:-/usr}"/{sbin,bin} \
		"${ED}${CPREFIX:-/usr}/share"/{man,locale,} \
		"${ED}${statedir}"/{run,lib/samba/private,lib/samba,lib,cache/samba,cache,} \
...
pkg_predeb() {
	for e in smbcacls smbcquotas smbget smbpasswd smbtree net rpcclient smbclient smbspool; do
		[[ -e "${ED}/${CPREFIX:-/usr}/bin/${e}" ]] \
			&& doverictld2 "${CPREFIX:-/usr}/bin/${e}" e - - - c
	done
}
