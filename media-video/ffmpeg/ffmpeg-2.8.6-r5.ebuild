...
inherit eutils flag-o-matic multilib multilib-minimal toolchain-funcs ${SCM} verictl2
...
IUSE="${IUSE} clip clip-deps clip-devstation clip-livecd"
...
RDEPEND="
	...
	!clip? ( opengl? ( >=virtual/opengl-7.0-r1[${MULTILIB_USEDEP}] ) )
	openssl? (
...
multilib_src_configure() {
	...
	set -- "${S}/configure" \
		...
		--shlibdir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)" \
...
pkg_predeb() {
	doverictld2 "${CPREFIX:-/usr}/bin/ffmpeg" e - - - c
	doverictld2 "${CPREFIX:-/usr}/bin/ffplay" e - - - c
}
