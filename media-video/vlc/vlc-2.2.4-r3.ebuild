...
inherit autotools flag-o-matic toolchain-funcs versionator virtualx ${SCM} verictl2
...
DESCRIPTION_FR="Lecteur multimédia - audio, vidéo, utilisation de la webcam"
CATEGORY_FR="Multimédia"
...
IUSE="${IUSE} clip clip-devstation clip-deps"
...
RDEPEND="
	...
	directfb? ( dev-libs/DirectFB:0 !clip-deps? ( sys-libs/zlib:0 ) )
	...
	gnome? ( gnome-base/gnome-vfs:2 !clip-deps? ( dev-libs/glib:2 ) )
	...
	libnotify? ( x11-libs/libnotify:0 x11-libs/gtk+:2 x11-libs/gdk-pixbuf:2 !clip-deps? ( dev-libs/glib:2 ) )
	...
	png? ( media-libs/libpng:0= !clip-deps? ( sys-libs/zlib:0 ) )
	postproc? (
	...
	sdl? ( >=media-libs/libsdl-1.2.10:0
		sdl-image? ( >=media-libs/sdl-image-1.2.10:0 !clip-deps? ( sys-libs/zlib:0 ) ) )
	...
	taglib? ( >=media-libs/taglib-1.9:0 !clip-deps? ( sys-libs/zlib:0 ) )
...
src_prepare() {
	...
	use clip && epatch "${FILESDIR}/${PN}-2.0.5-xcb-more-depth.patch"
	if use clip || use clip-devstation; then
	   	epatch "${FILESDIR}/${PN}-revert-win32-yadif-changes.patch"
	fi
...
src_configure() {
	...
	econf \
		${myconf} \
		--enable-vlc \
		--docdir=${CPREFIX:-/usr}/share/doc/${PF} \
...
pkg_postinst() {
	if [ "$ROOT" = "/" ] && [ -x "/usr/$(get_libdir)/vlc/vlc-cache-gen" ] ; then
		einfo "Running ${CPREFIX:-/usr}/$(get_libdir)/vlc/vlc-cache-gen on /usr/$(get_libdir)/vlc/plugins/"
		"${CPREFIX:-/usr}/$(get_libdir)/vlc/vlc-cache-gen" -f "${CPREFIX:-/usr}/$(get_libdir)/vlc/plugins/"
	else
...
pkg_predeb() {
	doverictld2 "${CPREFIX:-/usr}/bin/vlc" e - - - c
	cat >> "${D}/DEBIAN/postinst" <<EOF

if [[ "\${1}" == "configure" || "\${1}" == "abort-remove" ]]; then
	"${CPREFIX:-/usr}/$(get_libdir)/vlc/vlc-cache-gen" -f \\
		"${CPREFIX:-/usr}/$(get_libdir)/vlc/plugins/" \\
			|| echo " ! Failed to generate vlc cache !" >&2
fi
