...
inherit eutils waf-utils pax-utils fdo-mime gnome2-utils verictl2
...
src_prepare() {
	...
	# CLIP:
	epatch "${FILESDIR}/mpv-0.3.10-enlarge-your-buffer.patch"
}
...
src_configure() {
	...
	waf-utils_src_configure \
		...
		--confdir="${EPREFIX}${CPREFIX}"/etc/${PN} \
...
src_install() {
	...
	if use luajit; then
		pax-mark -m "${ED}${CPREFIX:-/usr}"/bin/mpv
	fi

	if use clip; then
		insinto /etc/${PN}
		newins "${FILESDIR}/mpv.conf.clip" mpv.conf
	fi
}
...
pkg_predeb() {
	doverictld2 "${CPREFIX:-/usr}/bin/${PN}" e - - - c
}
