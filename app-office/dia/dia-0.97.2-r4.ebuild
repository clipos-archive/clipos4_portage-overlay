...
DESCRIPTION_FR="Editeur de diagrammes"
...
IUSE="cairo doc python clip-deps"
...
src_configure() {
	...
	gnome2_src_configure \
		--exec-prefix=${EPREFIX}"${CPREFIX:-/usr}" \
...
src_install() {
	...
	sed -e 's|^Exec=dia|Exec=dia --integrated|' \
			...
			"${ED}${CPREFIX:-/usr}"/share/applications/dia.desktop \
			> "${ED}${CPREFIX:-/usr}"/share/applications/dia-integrated.desktop || die
}
