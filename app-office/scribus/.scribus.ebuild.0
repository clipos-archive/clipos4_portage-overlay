...
inherit cmake-utils fdo-mime python-single-r1 pax-utils

DESCRIPTION="Desktop publishing (DTP) and layout program"
DESCRIPTION_FR="Logiciel de Publication Assistée par Ordinateur"
...
COMMON_DEPEND="
	...
	media-libs/tiff:0
	net-print/cups
	!clip-deps? ( sys-libs/zlib[minizip] )
	virtual/jpeg
...
src_install() {
	...
	local lang file
	for lang in ${IUSE_LINGUAS}; do
		file="${ED}${CPREFIX:-/usr}"/share/scribus/translations/scribus.${lang}.qm
	...
	if ! use scripts; then
		rm "${ED}${CPREFIX:-/usr}"/share/scribus/scripts/*.py || die
	elif ! use tk; then
		rm "${ED}${CPREFIX:-/usr}"/share/scribus/scripts/{FontSample,CalendarWizard}.py || die
	fi

	python_fix_shebang "${ED}${CPREFIX:-/usr}"/share/scribus/scripts
	python_optimize "${ED}${CPREFIX:-/usr}"/share/scribus/scripts

	mv "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/{en,html} || die
	ln -sf html "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/en || die
	cat >> "${T}"/COPYING <<- EOF
	...
	if use clip; then
		# remove plugins containing TEXTRELs
		pax-mark L "${D}${CPREFIX:-/usr}/lib/scribus/plugins/libmeshdistortion.so" \
			|| die "paxctl failed"
		pax-mark L "${D}${CPREFIX:-/usr}/lib/scribus/plugins/libpathalongpath.so" \
			|| die "paxctl failed"
		pax-mark L "${D}${CPREFIX:-/usr}/bin/scribus" || die "paxctl failed"
	fi
}
