...
KEYWORDS="amd64 x86"
SLOT="4"
IUSE="xmp cdr calendar crypt debug expoblending gpssync +imagemagick ipod mediawiki mjpeg panorama redeyes scanner upnp videoslideshow vkontakte"
...
src_install() {
	...
	if use clip; then
		for f in dngconverter photolayoutseditor scangui; do
			if [[ -e "${ED}${CPREFIX:-/usr}/share/applications/kde4/${f}.desktop" ]]; then
				echo "NoDisplay=true" >> \
					"${ED}${CPREFIX:-/usr}/share/applications/kde4/${f}.desktop" \
						|| die ""
			fi
		done
		use expoblending || \
			rm -f "${ED}${CPREFIX:-/usr}/share/applications/kde4/panoramagui.desktop"
	fi
}
