...
IUSE="debug ffmpeg jack libsamplerate pulseaudio speex clip"
...
RDEPEND=">=media-libs/alsa-lib-${PV}:=[${MULTILIB_USEDEP}]
	...
	speex? ( >=media-libs/speex-1.1.99.4:=[${MULTILIB_USEDEP}] )
	abi_x86_32? (
...
multilib_src_install_all() {
	...
	if use pulseaudio; then
		...
		if use clip; then
			newins "${FILESDIR}"/51-pulseaudio-probe.conf.clip \
		...
		sed -i \
			-e "s:/lib/:/$(get_libdir)/:" \
			"${ED}${CPREFIX:-/usr}"/share/alsa/alsa.conf.d/51-pulseaudio-probe.conf || die #410261
	fi
