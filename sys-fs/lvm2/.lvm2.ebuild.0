...
EAPI=5
inherit autotools eutils linux-info multilib systemd toolchain-funcs udev flag-o-matic verictl2
...
RDEPEND="${DEPEND_COMMON}
	!clip? ( >=sys-apps/baselayout-2.2 )
	clip? ( !sys-fs/device-mapper )
...
src_install() {
	...
	use clip && INSTALL_TARGETS="install"
	for inst in ${INSTALL_TARGETS}; do
		emake DESTDIR="${D}" ${inst}
	done

	if use clip;  then
		rm -f "${D}/sbin/"vg* "${D}/sbin/"lv* "${D}/sbin"/pv* \
			"${D}/sbin/fsadm" "${D}/sbin/blkdeactivate"
		rm -fr "${D}/etc"
	else
...
pkg_predeb() {
	doverictld2 /sbin/dmsetup er 'SYS_ADMIN|MKNOD' - 'SYS_ADMIN|MKNOD' - 
}
