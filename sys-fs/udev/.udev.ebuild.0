...
IUSE="${IUSE} clip rm-deps clip-devstation clip-livecd"
...
COMMON_DEPEND="
	...
	acl? ( sys-apps/acl )
	!rm-deps? ( gudev? ( >=dev-libs/glib-2.34.3[${MULTILIB_USEDEP}] ) )
...
src_prepare() {
	...
	epatch "${FILESDIR}/${PN}-${PV}-mtd_probe_h.patch"
...
multilib_src_configure() {
	...
	econf_args=(
		...
		--libdir="${CPREFIX:-/usr}"/$(get_libdir)
		--with-html-dir="${CPREFIX:-/usr}"/share/doc/${PF}/html
		--with-rootprefix=
		--without-python
	...
	if multilib_is_native_abi; then
		econf_args+=(
			--with-rootlibdir=${CPREFIX}/$(get_libdir)
			$(use_enable acl)
		...
		econf_args+=(
			--with-rootlibdir=${CPREFIX:-/usr}/$(get_libdir)
			--disable-acl
			--disable-gtk-doc
			--disable-kmod
			--disable-selinux
...
multilib_src_install() {
	if !(use clip) && multilib_is_native_abi; then
...
multilib_src_install_all() {
	...
	rm -f \
		"${D}${CPREFIX}"/lib/udev/rules.d/99-systemd.rules \
		"${D}${CPREFIX:-/usr}"/share/doc/${PF}/LICENSE.*
