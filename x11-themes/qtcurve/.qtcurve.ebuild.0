...
LICENSE="LGPL-2+"
SLOT="0"
IUSE="+X gtk kde nls +qt4 qt5 windeco clip clip-rm"
REQUIRED_USE="gtk? ( X )
	windeco? ( kde X )
	|| ( gtk qt4 qt5 )"
...
RDEPEND="X? ( x11-libs/libxcb
	...
	kde? ( windeco? ( || ( >=kde-base/kdebase-workspace-${KDE_MINIMAL} 
				>=kde-base/kwin-${KDE_MINIMAL} ) )
			|| ( >=kde-base/kdebase-workspace-${KDE_MINIMAL} 
				>=kde-base/systemsettings-${KDE_MINIMAL} ) )
	clip? ( !x11-themes/qtcurve-qt4-nokde )
...
pkg_setup() {
	...
	use clip && PATCHES+=( "${FILESDIR}/${PN}-1.8.18-clip-prefix.patch" 
...
src_configure() {
	...
	use kde && mycmakeargs+=( -DQTC_KDE4_PREFIX="${CPREFIX:-/usr}" )
	use clip && mycmakeargs+=(-DAUTOMOC4_EXECUTABLE=/usr/bin/automoc4)
...
src_install() {
	...
	if use clip-rm && ! use kde; then
		rm -fr "${ED}${CPREFIX:-/usr}/share"
		rm "${ED}${CPREFIX:-/usr}/$(get_libdir)/"libqtcurve-utils*
	fi
}
