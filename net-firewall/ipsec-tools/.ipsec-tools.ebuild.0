...
inherit eutils flag-o-matic autotools linux-info verictl2
...
clip clip-devstation clip-livecd"
...
src_install() {
	...
	if use clip; then
		rm -fr "${D}/usr/lib"
		rm -f "${D}/usr/sbin"/{racoon,racoonctl,plainrsa-gen}
		rm -fr "${D}/var"
		rm -fr "${D}/etc/racoon"
	else
...
pkg_predeb() {
	# CAP_NET_ADMIN  
	doverictld2 /usr/sbin/setkey er NET_ADMIN - NET_ADMIN cNX
}
