...
inherit autotools-utils gnome2-utils deb
...
IUSE="X debug doc expat xml clip-deps"
...
RDEPEND="
	...
	virtual/jpeg
	xml? (
		!clip-deps? ( expat? ( dev-libs/expat ) )
...
src_configure() {
	...
	myeconfargs+=(
		...
		--disable-gd
		--with-sys-gd
		--with-gsfontdir="${EPREFIX}${CPREFIX:-/usr}"/share/ghostscript/fonts
		--with-fontdir="${EPREFIX}${CPREFIX:-/usr}"/share/fonts/urw-fonts/
		--with-docdir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}
		)
	autotools-utils_src_configure

	if [[ -n "${CPREFIX}" ]]; then
		find "${S}_build" -name Makefile | \
			xargs sed -i -e "s:GDK_PIXBUF_DIR = /usr/lib:GDK_PIXBUF_DIR = ${CPREFIX}/lib:g"
	fi
}
...
${base}/bin/gdk-pixbuf-query-loaders  > "${base}/$(get_libdir)/gdk-pixbuf-2.0/2.10.0/loaders.cache"
ENDSCRIPT
}
