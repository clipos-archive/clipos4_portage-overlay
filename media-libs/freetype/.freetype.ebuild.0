...
multilib_src_compile() {
	...
	if multilib_is_native_abi && use utils; then
		...
		emake \
			X11_PATH="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)" \
...
multilib_src_install_all() {
	if use fontforge; then
		...
		while read header; do
			mkdir -p "${ED}${CPREFIX:-/usr}/include/freetype2/internal4fontforge/$(dirname ${header})" || die
			cp ${header} "${ED}${CPREFIX:-/usr}/include/freetype2/internal4fontforge/$(dirname ${header})" || die
		done
	fi
