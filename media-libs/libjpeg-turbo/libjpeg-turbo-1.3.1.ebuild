...
# $Header: /var/cvsroot/gentoo-x86/media-libs/libjpeg-turbo/libjpeg-turbo-1.3.1.ebuild,v 1.4 2014/05/10 08:58:53 ssuominen Exp $
...
IUSE="java static-libs clip clip-devstation clip-livecd"

ASM_DEPEND="|| ( dev-lang/nasm dev-lang/yasm )"
PROVIDE="clip? ( media-libs/jpeg )"
COMMON_DEPEND="
	!media-libs/jpeg:0
	!clip? ( !media-libs/jpeg:62 )
...
multilib_src_configure() {
	...
	use clip && myconf+=( --with-jpeg8 )
	use clip-devstation && myconf+=( --with-jpeg8 )
	use clip-livecd && myconf+=( --with-jpeg8 )
...
multilib_src_install() {
	emake \
		...
		exampledir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF} \
		install

	if multilib_is_native_abi; then
		pushd "${WORKDIR}"/debian/extra >/dev/null
		emake \
			DESTDIR="${D}" prefix="${EPREFIX}${CPREFIX:-/usr}" \
			INSTALL="install -m755" INSTALLDIR="install -d -m755" \
			install
		popd >/dev/null

		if use java; then
			rm -rf "${ED}${CPREFIX:-/usr}"/classes
