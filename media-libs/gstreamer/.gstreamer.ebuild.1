...
IUSE="${IUSE} clip-deps clip clip-devstation"

RDEPEND="!clip-deps? ( >=dev-libs/glib-2.34.3:2[${MULTILIB_USEDEP}] )
...
multilib_src_configure() {
	...
	# helpers from there.
	local pname="GStreamer ebuild for Gentoo"
	local porig="http://packages.gentoo.org/package/media-libs/gstreamer"
	if use clip; then
		pname="GStreamer"
		porig="GStreamer"
	fi
	local checks="--enable-check"
	use clip && checks="--disable-check"
	use clip-devstation && checks="--disable-check"
	# Disable static archives, dependency tracking and examples
	# to speed up build time
	# Disable debug, as it only affects -g passing (debugging symbols), this must done through make.conf in gentoo
	ECONF_SOURCE=${S} \
	econf \
		--libexecdir="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir) \
		--disable-static \
		$(use_enable nls) \
		--disable-valgrind \
		--disable-examples \
		...
		--with-package-name="${pname}" \
		--with-package-origin="${porig}"
...
multilib_src_install() {
	...
	use orc && pax-mark -m "${ED}${CPREFIX:-/usr}/$(get_libdir)/gstreamer-${SLOT}/gst-plugin-scanner"
}
...
multilib_src_install_all() {
	...
	# Needed for orc-using gst plugins on hardened/PaX systems, bug #421579
	use orc && pax-mark -m "${ED}${cPREFIX:-usr}/bin/gst-launch-${SLOT}"
}
