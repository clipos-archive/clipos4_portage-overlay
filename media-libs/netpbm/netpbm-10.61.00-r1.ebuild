...
# $Header: /var/cvsroot/gentoo-x86/media-libs/netpbm/netpbm-10.61.00.ebuild,v 1.6 2013/07/28 19:32:53 aballier Exp $
...
IUSE="doc jbig jpeg jpeg2k png rle svga tiff X xml zlib clip-deps core-deps"
...
src_install() {
	[[ -n "${CPREFIX}" ]] && mkdir -p "${ED}/usr"
	# Subdir make targets like to use `mkdir` all over the place
	# without any actual dependencies, thus the -j1.
	emake -j1 package pkgdir="${ED}${CPREFIX:-/usr}" 

	[[ $(get_libdir) != "lib" ]] && mv "${ED}${CPREFIX:-/usr}"/lib "${ED}${CPREFIX:-/usr}"/$(get_libdir)

	# Remove cruft that we don't need, and move around stuff we want
	rm "${ED}${CPREFIX:-/usr}"/bin/{doc.url,manweb} || die
	rm -r "${ED}${CPREFIX:-/usr}"/man/web || die
	rm -r "${ED}${CPREFIX:-/usr}"/link || die
	rm "${ED}${CPREFIX:-/usr}"/{README,VERSION,config_template,pkginfo} || die
	dodir /usr/share
	mv "${ED}${CPREFIX:-/usr}"/man "${ED}${CPREFIX:-/usr}"/share/ || die
	mv "${ED}${CPREFIX:-/usr}"/misc "${ED}${CPREFIX:-/usr}"/share/netpbm || die
