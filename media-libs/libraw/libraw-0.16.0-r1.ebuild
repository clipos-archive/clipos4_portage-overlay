...
RDEPEND="jpeg? ( 
	!clip? ( >=virtual/jpeg-0-r2:0[${MULTILIB_USEDEP}] )
	clip? ( virtual/jpeg:0[${MULTILIB_USEDEP}] )
	)
...
src_install() {
	...
	if use clip; then
		# Too lazy to fix that, we don't need those files anyway
		rm -fr "${ED}/usr/share/cmake"
		rmdir "${ED}/usr/share"
	fi
}
