...
IUSE="jimtcl"
IUSE+=" clip-devstation clip-livecd clip"

COMMON_DEPEND="!clip? ( virtual/udev )
	virtual/libusb:1"
RDEPEND="${COMMON_DEPEND}
	jimtcl? ( dev-lang/jimtcl )
	!jimtcl? ( !clip? ( dev-lang/tcl:0 ) )" # usb_modeswitch script is tcl
...
src_install() {
	...
	if use clip; then
		rm -rf "${D}/lib/udev/"
		rm -f "${D}/usr/sbin/usb_modeswitch_dispatcher"
	fi
}
