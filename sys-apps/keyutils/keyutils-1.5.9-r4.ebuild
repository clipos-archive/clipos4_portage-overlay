...
DESCRIPTION_FR="Utilitaires de gestion de clés Linux"
...
IUSE="test clip clip-devstation clip-livecd"
...
DEPEND="!clip? ( !prefix? ( >=sys-kernel/linux-headers-2.6.11 ) )
...
multilib_src_compile() {
	...
	sed -i \
		...
		-e 's:-Werror::' \
		-e '/^BUILDFOR/s:=.*:=:' \
		-e "/^LIBDIR/s:=.*:=${CPREFIX:-/usr}/$(get_libdir):" \
...
multilib_src_install() {
	emake DESTDIR="${ED}" \
		ETCDIR=${CPREFIX:-/usr}/etc \
		BINDIR=${CPREFIX:-/usr}/bin \
		SBINDIR=${CPREFIX:-/usr}/sbin \
		SHAREDIR=${CPREFIX:-/usr}/share/keyutils \
		MANDIR=${CPREFIX:-/usr}/share/man \
		INCLUDEDIR=${CPREFIX:-/usr}/include \
		install || die
