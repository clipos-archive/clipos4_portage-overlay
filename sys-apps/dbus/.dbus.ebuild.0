...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/dbus/dbus-1.8.10.ebuild,v 1.11 2014/12/03 09:57:36 ago Exp $
...
IUSE="${IUSE} clip-deps clip"

CDEPEND="!clip-deps? ( >=dev-libs/expat-2 )
	selinux? (
...
src_prepare() {
	use clip && epatch "${FILESDIR}"/${PN}-1.8.6-clip-libc-enable-secure.patch
...
multilib_src_configure() {
	...
	myconf=(
		...
		--with-system-pid-file=${statedir}/run/dbus.pid
		--with-system-socket=${statedir}/run/dbus/system_bus_socket
...
multilib_src_install_all() {
	use clip || newinitd "${FILESDIR}"/dbus.initd dbus

	local statedir="/var"
	use clip && statedir="/var/shared"

	if use X && ! use clip; then
	...
	keepdir ${statedir}/lib/dbus
	# let the init script create the /var/run/dbus directory
	use clip || rm -rf "${ED}"/var/run
