...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/openrc/openrc-0.12.4.ebuild,v 1.6 2013/11/29 18:02:55 johu Exp $

EAPI=5

inherit eutils flag-o-matic multilib pam toolchain-funcs verictl2
...
IUSE="${IUSE} clip clip-livecd clip-devstation clip-dev-lxc"
...
PDEPEND="!clip? ( netifrc? ( net-misc/netifrc ) )"
...
src_prepare() {
	...
	if use clip-livecd; then
		epatch "${FILESDIR}/openrc-0.12.4-fix-no-umounts.patch"
		echo 'no_umounts="/mnt/livecd:/mnt/cdrom"' >>"${WORKDIR}/${P}/etc/rc.conf.in"
	fi
	if use clip-livecd || use clip-devstation; then
		sed -i -r 's/^(keymap=).*/\1"fr"/g' "${WORKDIR}/${P}/conf.d/keymaps"
	fi
	if use clip-dev-lxc; then
		echo 'rc_sys="lxc"' >>"${WORKDIR}/${P}/etc/rc.conf.in"
		echo 'rc_provide="net"' >>"${WORKDIR}/${P}/etc/rc.conf.in"
	fi

	if [[ -n "${IS_CLIP}" ]]; then
		epatch "${FILESDIR}/${PN}-0.12.4-clip.patch"
	fi
...
src_install() {
	...
	if [[ -n "${IS_CLIP}" ]]; then
		# In baselayout-clip
		rm -fr "${ED}/etc/conf.d" "${ED}/etc/init.d" "${ED}/etc/rc.conf"
		# Not used
		rm -fr "${ED}/$(get_libdir)/rc/net"
	else
...
pkg_predeb() {
	# For start-stop-daemon (symlink - avoid running doverictld2 on it 
	# directly or we'll end up hashing the SDK's rc...
	doverictld2 /sbin/rc er KILL KILL - P
}
