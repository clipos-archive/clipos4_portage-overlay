...
# $Header: /var/cvsroot/gentoo-x86/sys-apps/coreutils/coreutils-8.20.ebuild,v 1.9 2012/12/17 17:13:37 ago Exp $

EAPI="3"

inherit eutils flag-o-matic toolchain-funcs verictl2
...
IUSE="acl caps gmp nls selinux static userland_BSD vanilla xattr clip clip-core"
...
src_configure() {
	...
	# hostname    - net-tools
	local pkgr="Gentoo"
	local url="http://bugs.gentoo.org/"
	if use clip; then
		pkgr="CLIP"
		url="CLIP Bugzilla"
	fi
...
src_install() {
	...
	if use clip-core; then 
		...
		doexe ${D}/bin/date
		fowners root:4000 /mounts/admin_root/bin/date
		fperms 0750 /mounts/admin_root/bin/date
	fi
}
...
pkg_predeb() {
	# Authorized CAP_SYS_CHROOT
	doverictld2 /bin/chroot er SYS_CHROOT - SYS_CHROOT - ccsd
	# Authorized CAP_SYS_TIME
	doverictld2 /bin/date er SYS_TIME - SYS_TIME - ccsd
	# Authorized MKNOD (printer hotplug)
	doverictld2 /bin/mknod er MKNOD - MKNOD - ccsd

	use clip-core && \
		VERIEXEC_CTX=504 doverictld2 /mounts/admin_root/bin/date e SYS_TIME SYS_TIME - - ccsd
}
