...
src_prepare() {
	...
	if tc-is-cross-compiler ; then
		sed -i \
			-e "/^GROFFBIN=/s:=.*:=${EPREFIX}${CPREFIX:-/usr}/bin/groff:" \
			-e "/^TROFFBIN=/s:=.*:=${EPREFIX}${CPREFIX:-/usr}/bin/troff:" \
			-e '/^GROFF_BIN_PATH=/s:=.*:=:' \
			-e '/^GROFF_BIN_DIR=/s:=.*:=:' \
...
src_configure() {
	econf \
		--with-appresdir="${EPREFIX}${CPREFIX:-/usr}"/share/X11/app-defaults \
...
src_install() {
	...
	if ! use examples ; then
		...
		rm -f "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/pdf/mom-pdf.pdf
	fi
}
