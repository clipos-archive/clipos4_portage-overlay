...
src_prepare() {
	xorg-2_src_prepare
	if use clip; then
		epatch "${FILESDIR}/${PN}-1.5-fr-multi.patch"
		epatch "${FILESDIR}/${PN}-2.6-only-vtswitch.patch"
		epatch "${FILESDIR}/${PN}-2.9-clip-fix-parens-tigerVNC.patch"
	fi
	if use clip || use clip-devstation; then
		epatch "${FILESDIR}/${PN}-2.9-clip-extra-keymaps.patch"
	fi
