...
for p in /proc/*/cmdline; do
	cat "${p}" | grep -v autostart | grep -q rm_b_session.sh && let "num+=1"
done
if [[ ${num} -gt 1 ]]; then
	echo "rm_b_session.sh already active, exiting" >&2
	exit 0
fi
...
xauth -f "${NEW_AUTH}" generate "${DISPLAY}" . domain 3

viewer-launch "/${SERVER}" /bin/viewer.sh "Visionneuse RM_B" \
