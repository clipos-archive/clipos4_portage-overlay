...
#DESCRIPTION_FR="Pilotes d'imprimantes et de scanners HP"
DESCRIPTION_FR="Pilotes d'imprimantes HP"
CATEGORY_FR="Impression"
...
src_prepare() {
	...
	epatch "${WORKDIR}"
	if use clip; then
		epatch "${FILESDIR}/${P}-clip-change-installpath.patch"
	fi
	...
	local udevdir=${CPREFIX}$(get_udevdir)
...
src_configure() {
	...
	use clip && libexec="$(get_libdir)"
	local backenddir=$(cups-config --serverbin)/backend
	use clip && backenddir=/usr/local/lib/cups/backend
	local filterdir=$(cups-config --serverbin)/filter
	use clip && filterdir=/usr/local/lib/cups/filter

	econf \
		--libdir="${EPREFIX}${CPREFIX:-/usr}"/lib \
		--libexecdir="${EPREFIX}${CPREFIX:-/usr}"/${libexec} \
		...
		--with-cupsbackenddir=${backenddir} \
		--with-cupsfilterdir=${filterdir} \
		--with-docdir=${CPREFIX:-/usr}/share/doc/${PF} \
		--with-htmldir=${CPREFIX:-/usr}/share/doc/${PF}/html \
		${myconf} \
		${drv_build} \
		${minimal_build} \
...
src_install() {
	...
	rm -f "${D}"${CPREFIX:-/usr}/share/doc/${PF}/{copyright,README_LIBJPG,COPYING} || die
	rmdir --ignore-fail-on-non-empty "${D}"${CPREFIX:-/usr}/share/doc/${PF}/ || die
	...
	if use !minimal ; then
		python_export EPYTHON PYTHON
		python_optimize "${D}"${CPREFIX:-/usr}/share/hplip
	fi
}
