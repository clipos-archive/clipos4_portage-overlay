...
inherit check-reqs gnome2-utils java-pkg-2 java-vm-2 multiprocessing pax-utils prefix versionator virtualx verictl2
...
IUSE="${IUSE} clip clip-devstation clip-deps core-deps"

REQUIRED_USE="gtk? ( !headless-awt )"

# Ideally the following were optional at build time.
ALSA_COMMON_DEP="
	!core-deps? ( >=media-libs/alsa-lib-1.0 )"
CUPS_COMMON_DEP="
...
COMMON_DEP="
	...
	!clip? ( !clip-devstation? ( >=dev-util/systemtap-1 ) )
...
# clip change: fix the build dependency to dev-java/icedtea-bin:7
# runtime deps should be preserved by this change
...
PDEPEND="
	!clip-devstation? (
		webstart? ( dev-java/icedtea-web:0[icedtea7(+)] )
...
java_prepare() {
	...
	if use clip || use clip-devstation; then
		epatch "${FILESDIR}/${PN}-${PV}-clip-noinstall-tapset.patch"
	fi
...
src_configure() {
	...
	econf ${config} \
		...
		--prefix="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/icedtea${SLOT}" \
		--mandir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/icedtea${SLOT}/man" \
		--docdir="${EPREFIX}${CPREFIX:-/usr}/share/doc/${PF}" \
		--htmldir="${EPREFIX}${CPREFIX:-/usr}/share/doc/${PF}/html" \
...
src_compile() {
	...
	if use clip || use clip-devstation; then
		...
		# extract all the tarballs before patching
		emake -j 1 extract || die "make extract failed"
		emake -j 1 patch || die "make patch failed"
		cd "${S}"

		use clip && epatch "${FILESDIR}/${PN}-7.2.2.1-clip-mayexec.patch"
		epatch "${FILESDIR}/${PN}-7.2.6.4-clip-fix-sunec-libs-order.patch"
	
		## CLIP : respect LDFLAGS
		sed -i -e "s:LDFLAGS         = :LDFLAGS         = ${LDFLAGS} :" \
			"${S}/openjdk/jdk/make/common/Defs.gmk" \
			|| die
		sed -i -e "s:LDFLAGS_SED:${LDFLAGS}:" \
			"${S}/openjdk/hotspot/make/linux/makefiles/vm.make" \
			|| die
	fi
...
src_install() {
	...
	local dest="${CPREFIX:-/usr}/$(get_libdir)/icedtea${SLOT}"
	local ddest="${ED}${dest#/}"
	...
	einfo "Generating cacerts file from certificates in ${EPREFIX}${CPREFIX:-/usr}/share/ca-certificates/"
	mkdir "${T}/certgen" && cd "${T}/certgen" || die
	cp "${FILESDIR}/generate-cacerts.pl" . && chmod +x generate-cacerts.pl || die
	for c in "${EPREFIX}${CPREFIX:-/usr}"/share/ca-certificates/*/*.crt; do
	...
	use clip || set_java_env "${FILESDIR}/icedtea.env"
	use clip || java-vm_sandbox-predict /proc/self/coredump_filter

	if use clip; then
		einfo "Removing jdk"
		rm -fr "${ddest}/bin" "${ddest}/lib" "${ddest}/include"
		dosym "${dest}" "/usr/lib/java"
		find "${ddest}" -name "*.jar" -exec chmod +x '{}' \;
		dosym "${dest}/jre/bin/java" "/usr/bin/java"
		dosym "${dest}/jre/bin/java" "${dest}/bin/java"
		insinto "${dest}/jre/lib"
		doins "${FILESDIR}/swing.properties"
	fi
}
...
pkg_predeb() {
	doverictld2  ${CPREFIX:-/usr}/$(get_libdir)/icedtea${SLOT}/jre/bin/java e - - - cs
}
