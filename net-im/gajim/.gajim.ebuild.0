...
inherit autotools-utils python-r1 versionator verictl2 script

MY_PV=${PV/_/-}
MY_P="${PN}-${MY_PV}"

DESCRIPTION="Jabber client written in PyGTK"
DESCRIPTION_FR="Client de messagerie instantanée Jabber"
CATEGORY_FR="Réseau"
...
IUSE="crypt dbus gnome gnome-keyring kde idle jingle libnotify networkmanager nls spell +srv test X xhtml zeroconf clip kerberos"
...
src_install() {
	...
	rm "${D}${CPREFIX:-/usr}/share/doc/${PF}/README.html" || die
	dohtml README.html

	shebang_fix "${ED}${CPREFIX:-/usr}"/bin
	# Called from a shell script - not handled by shebang_fix
	sed -i -e "s:/usr/bin/python:${CPREFIX:-/usr}/bin/python:g" \
		"${ED}${CPREFIX:-/usr}"/bin/*
}

pkg_predeb() {
	doverictld2 "${CPREFIX:-/usr}/bin/gajim" es - - - cs
}
