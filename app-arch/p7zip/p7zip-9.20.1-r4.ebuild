...
DESCRIPTION_FR="Support des formats de compression 7-Zip"
...
IUSE="doc kde rar +pch static wxwidgets clip"
...
src_install() {
	...
	if use kde || use wxwidgets; then
		...
		if use kde; then
			...
			if use clip; then
				# We don't want the non translated upstream
				# desktop files, and the 'extract' actions
				# are redundant with ark
				doins "${FILESDIR}"/p7zip_compress.desktop
			else
