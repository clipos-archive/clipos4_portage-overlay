...
IUSE="afs bashlogger examples mem-scramble +net nls plugins +readline vanilla clip"
...
src_prepare() {
	...
	if use clip; then 
		epatch "${FILESDIR}"/${PN}-4.1-open_mayexec.patch
		eautoreconf
	fi
...
src_configure() {
	...
	# Check W^X constraints when opening or sourcing a command file
	use clip && myconf+=( ${myconf} --with-mayexec )
