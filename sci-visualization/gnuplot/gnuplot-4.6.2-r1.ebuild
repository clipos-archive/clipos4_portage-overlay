...
DESCRIPTION_FR="Logiciel de création de graphes"
...
IUSE="aqua bitmap cairo doc emacs examples +gd ggi latex lua plotutils qt4 readline svga thin-splines wxwidgets X xemacs clip"
...
TEXMF="${EPREFIX}${CPREFIX:-/usr}/share/texmf-site"
...
src_prepare() {
	...
	if use qt4; then
		append-ldflags -Wl,-rpath,"${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir)/qt4
	fi
...
src_configure() {
	...
	if use emacs; then
		...
		emacs=xemacs
		lispdir="${EPREFIX}${CPREFIX:-/usr}/lib/xemacs/site-packages/${PN}"
	else
		emacs=no
		lispdir=""
		if [[ -z ${PV%%*9999} ]]; then
	...
	econf \
		...
		$(use_with gd) \
		"$(use_with ggi ggi "${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)")" \
		"$(use_with ggi xmi "${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)")" \
		$(use_with lua) \
		"$(use_with plotutils plot "${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)")" \
...
src_install () {
	...
	if use examples; then
		...
		doins -r demo
		rm -f "${ED}${CPREFIX:-/usr}"/share/${PN}/${GP_VERSION}/demo/Makefile*
		rm -f "${ED}${CPREFIX:-/usr}"/share/${PN}/${GP_VERSION}/demo/binary*
	fi
