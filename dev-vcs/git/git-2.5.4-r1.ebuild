...
DESCRIPTION_FR="Client git (logiciel de gestion de version)"
...
CDEPEND="
	...
	curl? (
		net-misc/curl
		webdav? ( !clip-deps? ( dev-libs/expat ) )
	)
	emacs? ( virtual/emacs )
...
src_prepare() {
	...
	use clip && epatch "${FILESDIR}/git-2.0.4-clip-perl-inc-vendor.patch"
...
src_install() {
	...
	for i in \
		...
		cp -rf \
			"${S}"/contrib/${i} \
			"${ED}${CPREFIX:-/usr}"/share/${PN}/contrib \
	...
	if use perl && use cgi ; then
		...
		# distros
		dosym ${CPREFIX:-/usr}/share/gitweb ${CPREFIX:-/usr}/share/${PN}/gitweb
		...
		find "${ED}${CPREFIX:-/usr}"/lib64/perl5/ \
			-name .packlist \
			-exec rm \{\} \;
	else
		rm -rf "${ED}${CPREFIX:-/usr}"/share/gitweb
	fi

	if ! use subversion ; then
		rm -f "${ED}${CPREFIX:-/usr}"/libexec/git-core/git-svn \
			"${ED}${CPREFIX:-/usr}"/share/man/man1/git-svn.1*
	fi
...
pkg_predeb() {
	doverictld2  "${CPREFIX:-/usr}/bin/git" e - - - c
	doverictld2  "${CPREFIX:-/usr}/libexec/git-core/git-remote-ftps" e - - - c
	doverictld2  "${CPREFIX:-/usr}/libexec/git-core/git-http-push" e - - - c
	doverictld2  "${CPREFIX:-/usr}/libexec/git-core/git-http-fetch" e - - - c
}
