...
DESCRIPTION_FR="Client subversion (logiciel de gestion de version)"
...
COMMON_DEPEND=">=dev-db/sqlite-3.7.12
	...
	!core-deps? ( berkdb? ( >=sys-libs/db-4.0.14 ) )
...
src_prepare() {
	...
	# fperms broken on CLIP
	chmod +x build/transform_libtool_scripts.sh
	...
	# CLIP - Fix CPREFIX:
	epatch "${FILESDIR}/subversion-1.7.9-perl-prefix.patch"
}
...
src_configure() {
	...
	econf --libdir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)" \
		$(use_with apache2 apache-libexecdir) \
		$(use_with apache2 apxs "${APXS}") \
		$(use_with berkdb berkeley-db "db.h:${EPREFIX}${CPREFIX:-/usr}/include/db${SVN_BDB_VERSION}::db-${SVN_BDB_VERSION}") \
		$(use_with ctypes-python ctypesgen "${EPREFIX}${CPREFIX:-/usr}") \
...
src_install() {
	...
	if use java ; then
		...
		java-pkg_regso "${ED}${CPREFIX:-/usr}"/$(get_libdir)/libsvnjavahl*$(get_libname)
		java-pkg_dojar "${ED}${CPREFIX:-/usr}"/$(get_libdir)/svn-javahl/svn-javahl.jar
		rm -fr "${ED}${CPREFIX:-/usr}"/$(get_libdir)/svn-javahl/*.jar
	fi
...
PATH="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/subversion/bin"
ROOTPATH="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/subversion/bin"
EOF
		doenvd 80subversion-extras

		emake DESTDIR="${D}" toolsdir="${CPREFIX:-/usr}/$(get_libdir)/subversion/bin" install-tools
	...
	cd "${ED}${CPREFIX:-/usr}/share/locale"
	for i in * ; do
...
pkg_preinst() {
	...
	if use berkdb && [[ -f "${EROOT}${CPREFIX:-/usr}/bin/svn" ]]; then
		OLD_BDB_VERSION="$(scanelf -nq "${EROOT}${CPREFIX:-/usr}/$(get_libdir)/libsvn_subr-1$(get_libname 0)" | grep -Eo "libdb-[[:digit:]]+\.[[:digit:]]+" | sed -e "s/libdb-\(.*\)/\1/")"
		NEW_BDB_VERSION="$(scanelf -nq "${ED}${CPREFIX:-/usr}/$(get_libdir)/libsvn_subr-1$(get_libname 0)" | grep -Eo "libdb-[[:digit:]]+\.[[:digit:]]+" | sed -e "s/libdb-\(.*\)/\1/")"
...
pkg_config() {
	...
	if [[ -e "${SVN_REPOS_LOC}/repos" ]] ; then
		...
		"${EROOT}${CPREFIX:-/usr}/bin/svnadmin" create "${SVN_REPOS_LOC}/repos"
...
pkg_predeb() {
	doverictld2  "${CPREFIX:-/usr}/bin/svn" e - - - c
}
