...
RDEPEND="abi_x86_32? ( !app-emulation/emul-linux-x86-motif[-abi_x86_32(-)] )
	...
	!clip-deps? ( unicode? ( virtual/libiconv[${MULTILIB_USEDEP}] ) )
	!core-deps? ( 
...
multilib_src_install_all() {
	...
	if use examples; then
		...
		mv "${ED}${CPREFIX:-/usr}"/share/Xm/* "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/demos || die
	fi
	rm -rf "${ED}${CPREFIX:-/usr}"/share/Xm
