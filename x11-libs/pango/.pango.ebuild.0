...
# $Header: /var/cvsroot/gentoo-x86/x11-libs/pango/pango-1.36.8.ebuild,v 1.10 2015/01/02 12:11:17 ago Exp $

EAPI="5"
GCONF_DEBUG="yes"
GNOME2_LA_PUNT="yes"

inherit gnome2 multilib toolchain-funcs multilib-minimal deb
...
IUSE="X +introspection clip-deps core-deps"
...
pkg_predeb() {
	...
	multilib_pkg_predeb() {
		...
		local pango_conf="${CPREFIX}/${PANGO_CONFDIR}/pango.modules"
		dodir ${PANGO_CONFDIR}

		cat >> "${ED}/DEBIAN/postinst" << EOF 
if [[ "\${1}" == "configure" || "\${1}" == "abort-remove" ]]; then
	if ${CPREFIX:-/usr}/bin/pango-querymodules --system \
			"${CPREFIX:-/usr}"/$(get_libdir)/pango/1.8.0/modules/*.so \
									> "${pango_conf}.new" ; then
		mv "${pango_conf}.new" "${pango_conf}"
		chmod 644 "${pango_conf}"
	else
		echo "! error updating pango modules !" >&2
	fi
fi
EOF
	}
	multilib_foreach_abi multilib_pkg_predeb
}
