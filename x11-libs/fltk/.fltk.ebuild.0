...
IUSE="${IUSE} core-deps clip-deps clip"
...
FLTK_INCDIR=${EPREFIX}${CPREFIX:-/usr}/include/fltk
FLTK_LIBDIR=${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/fltk
...
src_configure() {
	...
	econf \
		...
		--libdir=${FLTK_LIBDIR} \
		--docdir="${EPREFIX}${CPREFIX:-/usr}/share/doc/${PF}/html" \
		--enable-largefile \
		--enable-shared \
		--enable-xdbe \
		--disable-localjpeg \
...
src_install() {
	...
	echo "FLTK_DOCDIR=${EPREFIX}${CPREFIX:-/usr}/share/doc/${PF}/html" >> 99fltk
	doenvd 99fltk
	...
	if ! use static-libs; then
		rm "${ED}${CPREFIX:-/usr}"/lib*/fltk/*.a || die
	fi
