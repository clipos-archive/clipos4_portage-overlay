...
# $Header: /var/cvsroot/gentoo-x86/x11-libs/gtk+/gtk+-3.12.2.ebuild,v 1.11 2014/10/11 11:39:16 maekke Exp $

EAPI="5"
GCONF_DEBUG="no"
GNOME2_LA_PUNT="yes"

inherit eutils flag-o-matic gnome2 multilib virtualx deb
...
src_install() {
	...
	insinto /etc/gtk-3.0
	if use clip; then
		newins "${FILESDIR}"/settings.ini.clip settings.ini
	else
	...
	if use aqua ; then
		for i in gtk+-3.0.pc gtk+-quartz-3.0.pc gtk+-unix-print-3.0.pc; do
			sed -e "s:Libs\: :Libs\: -framework Carbon :" \
				-i "${ED}${cPREFIX:-usr}"/$(get_libdir)/pkgconfig/$i || die "sed failed"
		done
	fi
}
...
pkg_preinst() {
	...
	local cache="${cPREFIX:-usr}/$(get_libdir)/gtk-3.0/3.0.0/immodules.cache"
...
pkg_postrm() {
	...
	if [[ -z ${REPLACED_BY_VERSIONS} ]]; then
		rm -f "${EROOT}${cPREFIX:-usr}"/$(get_libdir)/gtk-3.0/3.0.0/immodules.cache
	fi
}

pkg_predeb() {
	local GTK3_MODDIR="${CPREFIX:-/usr}/$(get_libdir)/gtk-3.0/3.0.0"

	einfo "Creating postinst"
	init_maintainer "postinst"
	cat >> "${ED}/DEBIAN/postinst" <<ENDSCRIPT
${CPREFIX:-/usr}/bin/gtk-query-immodules-3.0 > ${GTK3_MODDIR}/immodules.cache
