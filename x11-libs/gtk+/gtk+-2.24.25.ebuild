...
inherit eutils flag-o-matic gnome2 multilib virtualx autotools readme.gentoo multilib-minimal deb
...
IUSE="$IUSE clip clip-deps core-deps clip-livecd"
...
COMMON_DEPEND="
	...
	!clip-deps? ( >=dev-libs/glib-2.34.3:2[${MULTILIB_USEDEP}] )
...
set_gtk2_confdir() {
	if use clip; then
		GTK2_CONFDIR="${CPREFIX}/etc/gtk-2.0"
	else
		# An arch specific config directory is used on multilib systems
		GTK2_CONFDIR="${CPREFIX}/etc/gtk-2.0/${CHOST}"
	fi
}
...
multilib_src_install() {
	...
	use aqua && for i in gtk+-2.0.pc gtk+-quartz-2.0.pc gtk+-unix-print-2.0.pc; do
		sed -i -e "s:Libs\: :Libs\: -framework Carbon :" "${ED%/}${CPREFIX:-/usr}"/lib/pkgconfig/$i || die "sed failed"
	done
}
...
multilib_src_install_all() {
	...
	if use clip ; then
		...
		newins "${FILESDIR}/gtkrc.qt" gtkrc
	elif use clip-livecd ; then
		insinto ${GTK2_CONFDIR}
		newins "${FILESDIR}/gtkrc.qt.livecd" gtkrc
	else
		# see bug #133241
	...
	rm "${ED}${cPREFIX:-usr}"/bin/gtk-builder-convert
...
pkg_predeb() {
	...
	local confdir="${GTK2_CONFDIR}"
	local oldconf="${CPREFIX}/etc/gtk-2.0"
...
if [[ "\${1}" == "configure" || "\${1}" == "abort-remove" ]]; then
	if [[ -e ${confdir}/gdk-pixbuf.loaders ]]; then
		rm -f ${confdir}/gdk-pixbuf.loaders
	fi
	if [[ -e ${oldconf}/gdk-pixbuf.loaders ]]; then
		rm -f ${oldconf}/gdk-pixbuf.loaders
	fi
	if [[ -e ${oldconf}/gtk.immodules ]]; then
		rm -f ${oldconf}/gtk.immodules
	fi
	if [[ -e ${confdir}/gtk.immodules ]]; then
		rm -f ${confdir}/gtk.immodules
	fi
	${base}/bin/gtk-query-immodules-2.0 --update-cache \
		|| echo "Failed to update immodules !" >&2
fi
ENDSCRIPT
}
