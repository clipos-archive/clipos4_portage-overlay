...
RDEPEND="${CDEPEND}
	...
	clip? ( >=clip-layout/baselayout-clip-2.0.16-r1 )
"
DEPEND="${CDEPEND}
...
src_prepare() {
	...
	if use clip; then
		epatch "${FILESDIR}/${PN}-1.7.6-clip-chroot.patch"
		epatch "${FILESDIR}/${PN}-1.7.6-clip-log.patch"
	fi
...
src_configure() {
	...
	# CLIP: error-log-path, pid-path, lock-path
	# TODO: Add a nginx user
	./configure \
		--prefix="${EPREFIX}"/usr \
		--conf-path="${EPREFIX}"/etc/${PN}/${PN}.conf \
		--error-log-path="stderr" \
		--pid-path="${EPREFIX}"/tmp/${PN}.pid \
		--lock-path="${EPREFIX}"/tmp/${PN}.lock \
...
_install_jail() {
	...
	einfo "Creating the ${name} [${ctx}] jail"

	# Read-only directories
	local root="/vservers/${name}"
	for f in bin etc/{core,shared} lib sbin tmp usr var/lib/nginx srv/{dev,var/www}; do
		dodir "${root}/${f}"
	done

	# Devices
	local dev_dir="/mounts/vsdev/${name}"
	dodir "${dev_dir}"
	mknod -m 0666 "${ED}${dev_dir}/null" c 1 3
	mknod -m 0666 "${ED}${dev_dir}/full" c 1 7
	mknod -m 0666 "${ED}${dev_dir}/zero" c 1 5
	mknod -m 0666 "${ED}${dev_dir}/urandom" c 1 9
	ln -s "urandom" "${ED}${dev_dir}/random"

	# Symlinks
	views_create_etc_symlinks "${root}"
	dosym "../../../tmp" "${root}/var/lib/nginx/tmp"
	dosym "srv/dev" "${root}/dev"

	# Jail config
	insinto "/etc/jails/${name}"
	doins "${FILESDIR}/clip/${name}/jail"/*
	sed -i -r "s/@SIDE@/${side^^}/g" "${ED}/etc/jails/${name}/netfilter"

	local f
	for f in "${ED}/etc/jails/${name}"/*; do
		sed -i -r "s/@JAIL_NAME@/${name}/g;s/@JAIL_CONTEXT@/${ctx}/g" "$f"
	done

	# Nginx config
	for f in start stop; do
		exeinto "/etc/local.d"
		newexe "${FILESDIR}/clip/${name}/nginx.${f}" "${name}.${f}"
	done

	insinto "${root}/etc/nginx"
	doins "${FILESDIR}/clip/${name}/nginx.conf"
	sed -i -r "s/@JAIL_NAME@/${name}/g" "${ED}${root}/etc/nginx/nginx.conf"

	# Shared directory
	dodir "/etc/admin/${name}"
	fperms 775 "$_"
	fowners 0:4000 "$_"
}
...
src_install() {
	...
	if use clip; then
		local d
		for d in /etc/nginx /tmp /usr/html /var; do
			rm -rf "${D}${d}" || die
		done

		# Specific configurations
		rm -f "${D}/etc/nginx"/* || die

		if use clip-rb; then
			_install_jail rb_black 601 black
			_install_jail rb_red 602 red
		fi
...
pkg_predeb() {
	if use clip-rb; then
		local f=
		for f in "${ED}/etc/jails"/*/context; do
			VERIEXEC_CTX="$(<"$f")" doverictld2 /usr/sbin/nginx er \
				'SYS_CHROOT|SETUID|SETGID' \
				'SYS_CHROOT|SETUID|SETGID' - s
		done
	fi
}
