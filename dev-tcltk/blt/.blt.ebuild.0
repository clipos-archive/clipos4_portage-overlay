...
src_configure() {
	LC_ALL=C \
	econf \
		--exec-prefix="${CPREFIX:-/usr}" \
		--x-includes="${EPREFIX}/usr/include" \
		--x-libraries="${EPREFIX}/usr/$(get_libdir)" \
		--with-blt="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)" \
...
src_install() {
	sed \
		-e "s:\.\./src/bltwish:${EPREFIX}${CPREFIX:-/usr}/bin/bltwish:g" \
		-e "s:\.\./bltwish:${EPREFIX}${CPREFIX:-/usr}/bin/bltwish:g" \
		-e "s:/usr/local/bin/bltwish:${EPREFIX}${CPREFIX:-/usr}/bin/bltwish:g" \
		-e "s:/usr/local/bin/tclsh:${EPREFIX}${CPREFIX:-/usr}/bin/tclsh:g" \
	...
	dohtml html/*.html
	for f in `ls "${ED}${CPREFIX:-/usr}"/share/man/mann` ; do
		mv "${ED}${CPREFIX:-/usr}"/share/man/mann/${f} "${ED}${CPREFIX:-/usr}"/share/man/mann/${f/.n/.nblt}
	done

	# bug 259338 - dev-tcltk/blt-2.4z-r4 provides empty pkgIndex.tcl
	cp "${FILESDIR}"/pkgIndex.tcl "${ED}${CPREFIX:-/usr}"/$(get_libdir)/blt2.4/pkgIndex.tcl
	...
	use static-libs || \
		find "${ED}${CPREFIX:-/usr}"/$(get_libdir) -name "*.a" -print0 | \
		xargs -r -0 rm -fv
}
