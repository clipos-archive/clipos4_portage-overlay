...
IUSE="insecure_certs"
IUSE+=" clip clip-devstation clip-livecd clip-deps "
${PRECOMPILED} || IUSE+=" cacert"

DEPEND=""
if ${PRECOMPILED} ; then
	DEPEND+=" !clip-livecd? ( !clip-devstation? ( !clip? ( !<sys-apps/portage-2.1.10.41 ) ) )"
fi
# c_rehash: we run `c_rehash`
# debianutils: we run `run-parts`
RDEPEND="${DEPEND}
	!clip? ( !clip-livecd? ( !clip-devstation? ( app-misc/c_rehash ) ) )
	!clip-deps? ( sys-apps/debianutils )"
...
src_install() {
	...
	if ! ${PRECOMPILED} ; then
		# pour le CPREFIX de clip
		if [[ -n "${CPREFIX}" ]]; then
			mkdir -p "${D}/${CPREFIX}"
			mv "${D}"/usr/{sbin,share} "${D}/${CPREFIX}"
			mv "${D}"/etc "${D}/${CPREFIX}"
			sed -i "s:/usr:${CPREFIX}:g;
					s:/etc:${CPREFIX}/etc:g" \
					"${D}${CPREFIX}"/sbin/update-ca-certificates
		fi
	else
		# pour le cprefix de clip
		mv "${ED}${CPREFIX:-/usr}"/share/doc/{ca-certificates,${PF}} || die
	fi
...
export PATH="\${PATH}:${CPREFIX}/bin:${CPREFIX}/sbin"
# run  it twice, since first time might exit on some updates
