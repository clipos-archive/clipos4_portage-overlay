...
src_prepare() {
	if full_tarball; then
		epatch "${FILESDIR}/${PN}-3.16.0-full_tarball-build.patch"

		# Fix AC_CHECK_FUNCS.
		# https://mailinglists.sqlite.org/cgi-bin/mailman/private/sqlite-dev/2016-March/002762.html
		sed -e "s/AC_CHECK_FUNCS(.*)/AC_CHECK_FUNCS([fdatasync fullfsync gmtime_r isnan localtime_r localtime_s malloc_usable_size posix_fallocate pread pread64 pwrite pwrite64 strchrnul usleep utime])/" -i configure.ac || die "sed failed"
	else
		epatch "${FILESDIR}/${PN}-3.16.0-nonfull_tarball-build.patch"
...
multilib_src_compile() {
	emake HAVE_TCL="$(usex tcl 1 "")" TCLLIBDIR="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/${P}"
...
multilib_src_install() {
	emake DESTDIR="${D}" HAVE_TCL="$(usex tcl 1 "")" TCLLIBDIR="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/${P}" install
