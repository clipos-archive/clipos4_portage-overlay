...
src_configure() {
	...
	econf \
		...
		--with-gs-font-dir="${EPREFIX}${CPREFIX:-/usr}"/share/fonts/urw-fonts \
		...
		$(use_with djvu) \
		--with-dejavu-font-dir="${EPREFIX}${CPREFIX:-/usr}"/share/fonts/dejavu \
		$(use_with fftw) \
		$(use_with fpx) \
		$(use_with fontconfig) \
		...
		$(use_with tiff) \
		$(use_with webp) \
		$(use_with corefonts windows-font-dir "${EPREFIX}${CPREFIX:-/usr}"/share/fonts/corefonts) \
		$(use_with wmf) \
		$(use_with xml) \
		--${openmp}-openmp \
...
src_install() {
	...
	emake \
		DESTDIR="${D}" \
		DOCUMENTATION_PATH="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/html \
		install

	rm -f "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/html/{ChangeLog,LICENSE,NEWS.txt}
