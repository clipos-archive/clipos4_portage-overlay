...
IUSE="${IUSE} clip core-deps clip-devstation clip-livecd"
...
RDEPEND="
	...
	sane_backends_dell1600n_net? ( >=media-libs/tiff-3.9.7-r1[${MULTILIB_USEDEP}] )
	sane_backends_canon_pp? ( >=sys-libs/libieee1284-0.2.11-r3[${MULTILIB_USEDEP}] )
	sane_backends_hpsj5s? ( >=sys-libs/libieee1284-0.2.11-r3[${MULTILIB_USEDEP}] )
	sane_backends_mustek_pp? ( >=sys-libs/libieee1284-0.2.11-r3[${MULTILIB_USEDEP}] )
	usb? (
		!clip? (
			!clip-devstation? ( >=virtual/libusb-1-r1:1=[${MULTILIB_USEDEP}] )
			clip-devstation? ( virtual/libusb:0 )
		)
		clip? ( virtual/libusb:0 )
	)
	gphoto2? (
		>=media-libs/libgphoto2-2.5.3.1:=[${MULTILIB_USEDEP}]
		!core-deps? ( >=virtual/jpeg-0-r2:0=[${MULTILIB_USEDEP}] )
	)
...
pkg_setup() {
	enewgroup scanner
	use clip || enewuser saned -1 -1 -1 scanner
}
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		...
		use clip || udev_newrules tools/udev/libsane.rules 41-libsane.rules
...
multilib_src_install_all() {
	...
	keepdir ${vardir}/lib/lock/sane
	fowners root:scanner ${vardir}/lib/lock/sane
	fperms g+w ${vardir}/lib/lock/sane
	dodir /etc/env.d
