...
# $Header: /var/cvsroot/gentoo-x86/media-gfx/gimp/gimp-2.8.10-r1.ebuild,v 1.13 2014/05/17 13:57:02 ago Exp $
...
DESCRIPTION_FR="Logiciel avancé de manipulation d'images"
...
_clean_up_locales() {
	...
	for lang in ${LANGS}; do
		...
		rm -Rf "${ED}${CPREFIX:-/usr}"/share/locale/"${lang}" || die
	done
}
...
src_install() {
	...
	mv "${ED}${CPREFIX:-/usr}"/share/applications/{,zzz-}gimp.desktop || die

	prune_libtool_files --all

	# Prevent dead symlink gimp-console.1 from downstream man page compression (bug #433527)
	local gimp_app_version=$(get_version_component_range 1-2)
	mv "${ED}${CPREFIX:-/usr}"/share/man/man1/gimp-console{-${gimp_app_version},}.1 || die
