...
DESCRIPTION_FR="Outil de création de graphiques et diagrammes."
...
sed_Imakefile() {
	...
	vars2subs=( BINDIR="${EPREFIX}"${CPREFIX:-/usr}/bin
		...
		XFIGLIBDIR="${EPREFIX}"${CPREFIX:-/usr}/share/xfig
		XFIGDOCDIR="${EPREFIX}${CPREFIX:-/usr}/share/doc/${PF}"
		MANDIR="${EPREFIX}${CPREFIX:-/usr}/share/man/man\$\(MANSUFFIX\)"
		XAPPLOADDIR="${EPREFIX}"${CPREFIX:-/usr}/etc/X11/app-defaults
		"CC=$(tc-getCC)" )
...
src_prepare() {
	...
	sed -e "s:/usr/lib/X11/xfig:${EPREFIX}${CPREFIX:-/usr}/share/doc/${PF}:" \
...
src_compile() {
	...
	emake CC="$(tc-getCC)" LOCAL_LDFLAGS="${LDFLAGS}" CDEBUGFLAGS="${EXTCFLAGS}" \
		USRLIBDIR="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir)
}
