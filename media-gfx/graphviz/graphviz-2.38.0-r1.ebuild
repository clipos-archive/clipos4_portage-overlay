...
DESCRIPTION_FR="Ensemble d'outils de manipulation de graphes (dot, etc.)"
...
src_install() {
	...
	emake DESTDIR="${D}" \
		txtdir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF} \
		htmldir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/html \
		htmlinfodir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/html/info \
		pdfdir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/pdf \
		pkgconfigdir="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir)/pkgconfig \
		install

	use examples || rm -rf "${ED}"/usr/share/graphviz/demo

	if ! use static-libs; then
		find "${ED}${CPREFIX:-/usr}"/$(get_libdir)/ -name '*.la' -delete || die
	fi
...
pkg_predeb() {
	einfo "Running dot -c..."
	LD_LIBRARY_PATH="${ED}${CPREFIX:-/usr}/$(get_libdir)" "${ED}${CPREFIX:-/usr}"/bin/dot -c || die 
}
