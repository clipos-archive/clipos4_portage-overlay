...
src_install() {
	...
	echo "default" > "${ED}${CPREFIX:-/usr}"/share/qt4/graphicssystems/raster || die
	touch "${ED}${CPREFIX:-/usr}"/share/qt4/graphicssystems/native || die

	if use clip; then
		dodir "${QTSYSCONFDIR}"
		insinto "${QTSYSCONFDIR}"
		doins "${FILESDIR}"/Trolltech.conf
	else
