...
EAPI=5
inherit nsplugins toolchain-funcs versionator multilib multilib-minimal pax-utils

DESCRIPTION="Adobe Flash Player"
DESCRIPTION_FR="Plugin propriétaire Adobe Flash. Offre une meilleure compatibilité que le plugin libre Gnash installé par défaut, au prix d'une sécurité réduite. <font color=\"red\"><b>Attention :</b> ce paquetage introduit potentiellement de nombreuses vulnérabilités de sécurité, qui ne peuvent pas être corrigées du fait de sa nature propriétaire.</font> <b>N.B. : il est souhaitable de ne pas installer ce paquetage en même temps que Gnash</b>."
...
"
IUSE="debug kde selinux cpu_flags_x86_sse2 clip clip-deps"
REQUIRED_USE="
	cpu_flags_x86_sse2
	debug? ( abi_x86_32 )
...
RDEPEND="
	...
	abi_x86_32? (
		amd64? (
			...
			!clip-deps? ( >=dev-libs/glib-2.34.3:2[abi_x86_32(-)] )
...
multilib_src_install() {
	...
	use clip && pax-mark L libflashplayer.so
	...
	if multilib_is_native_abi; then
		if use kde; then
			...
			if use clip; then
				# Carries an invalid rpath anyway
				chrpath -r "${CPREFIX:-/usr}/lib" usr/${pkglibdir}/kde4/kcm_adobe_flash_player.so
			fi
