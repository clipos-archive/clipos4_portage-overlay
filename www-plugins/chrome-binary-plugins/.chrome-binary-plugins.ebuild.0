...
DESCRIPTION_FR="Plugins flash propriétaires pour chromium (non fonctionnel("
...
IUSE="clip +flash +widevine"
RESTRICT="bindist mirror strip"

for x in 0 beta stable unstable; do
	if [[ ${SLOT} != ${x} ]]; then
		RDEPEND+=" !clip? ( !${CATEGORY}/${PN}:${x} )"
	fi
done
...
src_install() {
	...
	if use flash; then
		...
		flapper="${CPREFIX:-/usr}/$(get_libdir)/chromium-browser/PepperFlash/libpepflashplayer.so"
		...
		doins pepper-flash
		if use clip; then
			cat > "${T}/chromium-unsafe" <<EOF
#!/bin/sh

exec chromium --disable-setuid-sandbox --no-sandbox --ppapi-flash-path=$flapper \\
		--ppapi-flash-version=$version "\${@}"
EOF
			exeinto /usr/bin
			doexe "${T}/chromium-unsafe"
		fi
	fi
}
