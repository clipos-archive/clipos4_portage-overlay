...
IUSE="${IUSE} clip-devstation clip-livecd clip clip-deps"
...
COMMON_DEPEND="
	...
	!clip-deps? ( ncurses? ( >=sys-libs/ncurses-5.2:0=[${MULTILIB_USEDEP}] ) )
	...
	!clip-deps? ( nls? ( sys-devel/gettext[${MULTILIB_USEDEP}] ) )
...
RDEPEND="${COMMON_DEPEND}
	...
	!clip-deps? ( perl? ( dev-lang/perl dev-perl/XML-Simple ) )
...
QA_DESKTOP_FILE="${cPREFIX:-usr}/share/applications/wine-browsedrive.desktop
${cPREFIX:-usr}/share/applications/wine-notepad.desktop
${cPREFIX:-usr}/share/applications/wine-uninstaller.desktop
${cPREFIX:-usr}/share/applications/wine-winecfg.desktop"
...
src_prepare() {
	...
	if use clip; then 
		epatch "${FILESDIR}/${PN}-1.7.6-clip-mayexec.patch"
		epatch "${FILESDIR}/${PN}-1.7.6-clip-nomenu.patch"
		epatch "${FILESDIR}/${PN}-1.8-clip-noexec.patch"
	fi
}
...
multilib_src_install_all() {
	...
	if ! use perl ; then # winedump calls function_grep.pl, and winemaker is a perl script
		rm "${D}${CPREFIX:-/usr}"/bin/{wine{dump,maker},function_grep.pl} "${D}${CPREFIX:-/usr}"/share/man/man1/wine{dump,maker}.1 || die
	fi

	use abi_x86_32 && pax-mark psmr "${D}${CPREFIX:-/usr}"/bin/wine{,-preloader} #255055
	use abi_x86_64 && pax-mark psmr "${D}${CPREFIX:-/usr}"/bin/wine64{,-preloader}

	if ( use clip && use abi_x86_64 ) || ( use abi_x86_64 && ! use abi_x86_32 ); then
	...
	if [[ -n "${CPREFIX}" && "${CPREFIX}" != "/usr" ]]; then
		...
		mv "${D}/usr/share/applications/"* "${D}${CPREFIX}/share/applications/"
		rm -fr "${D}/usr/share/applications"
		mv "${D}/usr/share/"* "${D}${CPREFIX}/share/"
		rm -fr "${D}/usr/share"
		mv "${D}/etc/"* "${D}${CPREFIX}/etc/"
		rm -fr "${D}/etc"
	fi
	if use clip; then
		rm -fr "${D}${CPREFIX:-/usr}/share/applications"
	fi
}
