...
src_prepare() {
	...
	if use clip; then
		epatch "${FILESDIR}"/${PN}-5.0-clip-no-system-dbus.patch
		epatch "${FILESDIR}"/${PN}-2.1-clip-setgid.patch
	fi
...
multilib_src_install_all() {
	...
	use X || rm "${ED}${CPREFIX:-/usr}"/bin/start-pulseaudio-x11
	...
	use system-wide || rm "${ED}${CPREFIX}"/etc/dbus-1/system.d/pulseaudio-system.conf

	prune_libtool_files --all

	if use clip; then
		# root:audio
		fowners 0:18 "/usr/bin/pulseaudio"
		fperms 2755 "/usr/bin/pulseaudio"

		if [[ -n "${CPREFIX}" ]]; then
			rm -fr "${ED}/usr/share/bash-completion"
			rmdir "${ED}/usr/share"
		fi
	fi
}
