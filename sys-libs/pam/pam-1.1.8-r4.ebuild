...
clip clip-devstation clip-bare"
...
DEPEND="${RDEPEND}
	...
	clip-devstation? (
		>=sys-apps/shadow-4.1.5.1-r4
		>=sys-auth/pambase-20120417-r4
	)"
PDEPEND="!clip? ( !clip-devstation? ( sys-auth/pambase ) )
...
src_prepare() {
	if use clip; then
		# Replace some /etc paths with /etc/core
		epatch "${FILESDIR}/${PN}-1.1.0-clip-core-passwd.patch"

		# modify pam_wheel to verify whether the *login* provided by the user belongs to an arbitrary group
		epatch "${FILESDIR}/clip-wheel.patch"
	fi
...
multilib_src_install_all() {
	...
	# Need to be suid
	use clip || fperms u+s /sbin/unix_chkpwd
	...
	if use clip && use !clip-devel; then
		...
		newins "${FILESDIR}/access.conf.clip" "access.conf"
		if use clip-bare; then
			line_prefix='#'
		fi
		sed -i -e "s:@clip_bare_protect@:${line_prefix}:g" "${D}"/etc/security/access.conf
	fi
