...
multilib_src_install() {
	...
	if multilib_is_native_abi && use java; then
		java-pkg_regso "${D}${CPREFIX:-/usr}"/"$(get_libdir)"/libdb_java*.so
		java-pkg_dojar "${D}${CPREFIX:-/usr}"/"$(get_libdir)"/*.jar
		rm -f "${D}${CPREFIX:-/usr}"/"$(get_libdir)"/*.jar
	fi
}
...
multilib_src_install_all() {
	...
	if [[ -f "${D}${CPREFIX:-/usr}"/bin/berkeley_db_svc ]] ; then
		mv "${D}${CPREFIX:-/usr}"/bin/berkeley_db_svc \
			"${D}${CPREFIX:-/usr}"/sbin/berkeley_db"${SLOT/./}"_svc || die
	fi
}
