...
# $Header: /var/cvsroot/gentoo-x86/sys-libs/libcap/libcap-2.22-r2.ebuild,v 1.11 2014/06/18 20:50:55 mgorny Exp $
...
IUSE="pam clip-rm clip-devstation clip-livecd"
...
multilib_src_install() {
	...
	if [[ -n "${CPREFIX}" ]]; then
		emake install lib="$(get_libdir)" prefix="${CPREFIX}" DESTDIR="${ED}"
	else
	...
	rm -rf "${ED}${CPREFIX:-/usr}"/$(get_libdir)/security

	if use clip-rm; then
		rm -fr "${ED}${CPREFIX}/sbin" 
	fi
