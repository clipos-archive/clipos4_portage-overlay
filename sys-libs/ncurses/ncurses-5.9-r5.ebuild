...
IUSE="ada +cxx debug doc gpm minimal profile static-libs tinfo trace unicode clip"
...
do_configure() {
	ECONF_SOURCE=${S}
	local sysconf_terminfo="${EPREFIX}/etc/terminfo:${EPREFIX}/usr/share/terminfo"
	use clip && sysconf_terminfo="/etc/shared/terminfo"
	...
	local conf=(
		...
		# src_install() ...
		--with-terminfo-dirs="${sysconf_terminfo}:/usr/share/terminfo"
...
multilib_src_install_all() {
	local sysconf_terminfo="/etc/terminfo"
	use clip && sysconf_terminfo="/etc/shared/terminfo"
...
#	if ! use berkdb ; then
		...
		do
			...
			if [[ -n ${termfile} ]] ; then
				dodir ${sysconf_terminfo}/${basedir}
				mv ${termfile} "${D}"${sysconf_terminfo}/${basedir}/
				dosym ../../../..${sysconf_terminfo}/${basedir}/${x} \
