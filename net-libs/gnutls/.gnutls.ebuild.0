...
IUSE+=" clip-deps core-deps clip-devstation clip"
...
RDEPEND="
	...
	>=dev-libs/libtasn1-4.3[${MULTILIB_USEDEP}]
	>=dev-libs/nettle-2.7:=[gmp,${MULTILIB_USEDEP}]
	>=dev-libs/gmp-5.1.3-r1[${MULTILIB_USEDEP}]
	!clip? ( !clip-devstation? ( sys-devel/autogen ) )
...
multilib_src_install_all() {
	...
	if use doc; then
		...
		rm -fr "${ED}${CPREFIX:-/usr}/share/doc/${PF}/html"
	fi
	...
	# Man pages take a huge amount of space for not much
	# use on CLIP.
	if use clip; then
		rm -fr "${ED}${CPREFIX:-/usr}/share/man"
	fi
}
