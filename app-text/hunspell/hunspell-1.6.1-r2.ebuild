...
IUSE="ncurses nls readline static-libs clip-deps core-deps clip"
...
RDEPEND="
	...
	!core-deps? ( readline? ( sys-libs/readline:= ) )"
DEPEND="${RDEPEND}
	!clip-deps? (
		sys-devel/gettext
	)"
...
src_prepare() {
	use clip && epatch "${FILESDIR}/${PN}-1.6.1-clip-dict-paths.patch"
	default
	eautoreconf
}
...
src_install() {
	...
	#342449
	pushd "${ED}${CPREFIX:-/usr}"/$(get_libdir)/ >/dev/null
