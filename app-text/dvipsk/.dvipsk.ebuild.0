...
src_install() {
	emake DESTDIR="${D}" prologdir="${EPREFIX}${CPREFIX:-/usr}/share/texmf/dvips/base" install || die

	dodir /usr/share # just in case
	cp -pR "${WORKDIR}"/texmf "${ED}${CPREFIX:-/usr}/share/" || die "failed to install texmf trees"
	cp -pR "${WORKDIR}"/texmf-dist "${ED}${CPREFIX:-/usr}/share/" || die "failed to install texmf trees"
	if use source ; then
