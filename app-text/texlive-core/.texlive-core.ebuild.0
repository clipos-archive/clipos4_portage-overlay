...
inherit eutils flag-o-matic toolchain-funcs libtool texlive-common pax-utils deb
...
SRC_URI="${SRC_URI} mirror://gentoo/${PN}-patches-${PATCHLEVEL}.tar.xz
	mirror://gentoo/${PN}-extra-${PV}.tar.gz"
...
IUSE="cjk X doc source tk xetex clip clip-deps core-deps"
...
COMMON_DEPEND="${MODULAR_X_DEPEND}
	...
	!core-deps? ( >=media-libs/libpng-1.2.43-r2:0 )
	clip? ( >=app-text/poppler-0.32.0 )
...
src_install() {
	...
	cp -pR texmf{,-dist} "${D}${CPREFIX:-/usr}/share/" || die "failed to install texmf trees"
	cp -pR "${WORKDIR}"/tlpkg "${D}${CPREFIX:-/usr}/share/" || die "failed to install tlpkg files"

	if use clip; then
		newsbin "${FILESDIR}/texmf-update2010-clip" texmf-update
		epatch "${FILESDIR}/mktexlsr-tty-patch-clip.patch"
	else
	...
	use doc || rm -rf "${D}${CPREFIX:-/usr}/share/texmf/doc"
	use doc || rm -rf "${D}${CPREFIX:-/usr}/share/texmf-dist/doc"

	dodir /etc/env.d
	echo 'CONFIG_PROTECT_MASK="${CPREFIX}/etc/texmf/web2c ${CPREFIX}/etc/texmf/language.dat.d ${CPREFIX}/etc/texmf/language.def.d ${CPREFIX}/etc/texmf/updmap.d"' > "${D}${CPREFIX}/etc/env.d/98texlive"
	# populate /etc/texmf
	...
	mv "${D}${C_TEXMF_PATH}/web2c/updmap.cfg" "${D}${CPREFIX}/etc/texmf/updmap.d/00updmap.cfg" || die "moving updmap.cfg failed"

	# Remove fmtutil.cnf, it will be regenerated from /etc/texmf/fmtutil.d files
	# by texmf-update
	rm -f "${D}${C_TEXMF_PATH}/web2c/fmtutil.cnf"
	# Remove bundled and invalid updmap.cfg
	rm -f "${D}${CPREFIX:-/usr}/share/texmf-dist/web2c/updmap.cfg"
	...
	# CLIP : add /usr/local/bin to texmf-update script
	if use clip; then
		/sbin/paxctl -L ${D}${CPREFIX:-/usr}/bin/mf || die "paxctl failed"
		/sbin/paxctl -L ${D}${CPREFIX:-/usr}/bin/mf-nowin || die "paxctl failed"
	fi

	# Remove texdoctk if we don't want it
	if ! use tk ; then
		rm -f "${D}${CPREFIX:-/usr}/bin/texdoctk" "${D}${CPREFIX:-/usr}/share/texmf/scripts/tetex/texdoctk.pl" "${D}${CPREFIX:-/usr}/share/man/man1/texdoctk.1" || die "failed to remove texdoc tk!"
	fi

	# Rename mpost to leave room for mplib
	mv "${D}${CPREFIX:-/usr}/bin/mpost" "${D}${CPREFIX:-/usr}/bin/mpost-${P}"
	dosym "mpost-${P}" /usr/bin/mpost

	# Ditto for pdftex
	mv "${D}${CPREFIX:-/usr}/bin/pdftex" "${D}${CPREFIX:-/usr}/bin/pdftex-${P}"
...
echo "* Updating texmf *"
${CPREFIX:-/usr}/sbin/texmf-update || echo "! texmf-update failed !" >&2
