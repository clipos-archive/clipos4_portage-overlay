...
inherit eutils gnome2 autotools verictl2

DESCRIPTION="A international dictionary supporting fuzzy and glob style matching"
DESCRIPTION_FR="Dictionnaire Français et international"
CATEGORY_FR="Utilitaires"
HOMEPAGE="http://code.google.com/p/stardict-3/"
CLIP_DICTS="stardict-clip-dicts-1.0"
SRC_URI="http://${PN}-3.googlecode.com/files/${P}.tar.bz2
	clip? ( mirror://clip/${CLIP_DICTS}.tar.bz2 )
...
IUSE="espeak gnome gucharmap qqwry pronounce spell tools clip clip-deps"
...
src_prepare() {
	epatch \
		...
		"${FILESDIR}"/${P}-gmodule.patch

	# Do not enable net lookup by default
	# - Now disabled by default upstream
	#if use clip; then
	#	sed -i -e 's:enable_netdict", true:enable_netdict", false:' \
	#		"${S}/src/conf.cpp" || die "sed failed"
	#fi
...
src_install() {
	...
	if use clip; then
		mv "${WORKDIR}/${CLIP_DICTS}"/* \
				"${D}${CPREFIX:-/usr}/share/stardict/dic/" \
			|| die "failed to move CLIP dictionaries"
	fi
	if use qqwry; then
...
pkg_predeb() {
	doverictld2  ${CPREFIX:-/usr}/bin/stardict e - - - c
}
