...
IUSE_DAEMON="${IUSE_DAEMON} clip clip-deps"
IUSE_BACKEND="+berkdb"
...
CDEPEND="icu? ( dev-libs/icu:= )
	...
	!minimal? (
		...
		!clip-deps? ( perl? ( dev-lang/perl[-build(-)] ) )
...
multilib_src_configure() {
	...
	if ! use minimal && ! use clip && multilib_is_native_abi; then
	...
	if ! use clip; then
		...
		myconf+=(
			--enable-dynamic
			--disable-local --enable-proctitle
			--disable-ipv6 --without-cyrus-sasl
			--without-fetch
			--with-tls=openssl --without-yielding-select
			--without-mp
			--disable-debug
		)
	fi
	...
	STRIP=/bin/true \
	econf \
		--libexecdir="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir)/openldap \
		"${myconf[@]}"
	emake depend
}
	...
	if ! use minimal && ! use clip && multilib_is_native_abi ; then
		...
		if use smbkrb5passwd ; then
			...
			emake \
				...
				CC="${CC}" libexecdir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/openldap"
		fi
		...
		if use overlays ; then
			...
			emake \
				LDAP_BUILD="${BUILD_DIR}" \
				CC="${CC}" libexecdir="${CPREFIX:-/usr}/$(get_libdir)/openldap"
		fi
		...
		if use kerberos ; then
			...
			"${lt}" --mode=link --tag=CC \
				...
				${CFLAGS} \
				${LDFLAGS} \
				-rpath "${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir)/openldap/openldap \
				-o pw-kerberos.la \
...
multilib_src_install() {
	...
	if ! use minimal && ! use clip && multilib_is_native_abi; then
		...
		# use our config
		rm "${ED}${CPREFIX}/"etc/openldap/slapd.conf
		insinto /etc/openldap
		newins "${FILESDIR}"/${PN}-2.3.34-slapd-conf slapd.conf
		configfile="${ED}${CPREFIX}/"etc/openldap/slapd.conf

		# populate with built backends
		ebegin "populate config with built backends"
		for x in "${ED}${cPREFIX:-usr}"/$(get_libdir)/openldap/openldap/back_*.so; do
		...
		if [[ $(get_libdir) != lib ]]; then
			sed -e "s,/usr/lib/,/usr/$(get_libdir)/," -i \
				"${ED}${CPREFIX}"/etc/init.d/slapd \
				"${ED}${CPREFIX:-/usr}"/lib/systemd/system/slapd.service || die
		fi
		...
		if use cxx ; then
			...
			cd "${BUILD_DIR}/contrib/ldapc++" || die
			emake DESTDIR="${D}" libexecdir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/openldap" install
		...
		if use smbkrb5passwd ; then
			...
			emake DESTDIR="${D}" \
				LDAP_BUILD="${BUILD_DIR}" \
				libexecdir="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/openldap" install
		...
		if use overlays ; then
			...
			emake DESTDIR="${D}" \
				LDAP_BUILD="${BUILD_DIR}" \
				libexecdir="${CPREFIX:-/usr}/$(get_libdir)/openldap" install
		...
		for l in */*.la; do
			"${lt}" --mode=install cp ${l} \
				"${ED}${CPREFIX:-/usr}"/$(get_libdir)/openldap/openldap || \
	...
	if use clip; then
	  rm -f "${ED}${CPREFIX}/etc/openldap/ldap.conf"
	  dodir /etc/openldap
	  insopts -o 0 -g 0 -m0444
	  insinto /etc/openldap
	  doins "${FILESDIR}/ldap.conf"
	fi
}
...
pkg_postinst() {
	if ! use minimal && ! use clip; then
		...
		use prefix || chown root:ldap "${EROOT}${CPREFIX}/"etc/openldap/slapd.conf{,.default}
		chmod 0640 "${EROOT}${CPREFIX}/"etc/openldap/slapd.conf{,.default}
