...
FONT_OPTIONS="--with-otf-fontdir=${EPREFIX}${cPREFIX:-usr}/share/fonts/OTF
				--with-ttf-fontdir=${EPREFIX}${CPREFIX:-usr}/share/fonts/TTF"

src_install() {
	xorg-2_src_install
	# TTF fonts are not supposed to be installed.
	# Also fixes file collision per bug #309689
	rm -rf "${D}/${EPREFIX}${cPREFIX:-usr}/share/fonts/TTF"
}
