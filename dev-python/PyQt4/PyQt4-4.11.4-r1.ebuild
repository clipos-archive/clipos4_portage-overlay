...
IUSE="${IUSE} clip clip-devstation"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	sql? ( X )
	testlib? ( X )
...
src_configure() {
	configuration() {
		local myconf=(
			...
			--bindir="${EPREFIX}${CPREFIX:-/usr}/bin"
			--destdir="$(python_get_sitedir)"
			--sipdir="${EPREFIX}${CPREFIX:-/usr}/share/sip"
			--qsci-api
			--enable=QtCore
			--enable=QtNetwork
			--enable=QtXml
...
src_install() {
	installation() {
		...
		if ! use clip; then
			python_doexe "${tmp_root}${EPREFIX}${CPREFIX:-/usr/}"/bin/pyuic4
	...
	if [[ -n "${CPREFIX}" && "${CPREFIX}" != "/usr" ]]; then
		dodir "/usr/$(get_libdir)/python2.7/site-packages/"
		mv "${ED}/usr/$(get_libdir)/python2.7/site-packages/dbus" \
			"${ED}${CPREFIX}/$(get_libdir)/python2.7/site-packages/dbus" \
			|| die 
		if [[ -d "${ED}/usr/$(get_libdir)/qt4" ]]; then
			mv "${ED}/usr/$(get_libdir)/qt4" "${ED}${CPREFIX}/$(get_libdir)/" \
				|| die 
		fi
		if [[ -d "${ED}/usr/share/qt4" ]]; then
			mv "${ED}/usr/share/qt4" "${ED}${CPREFIX}/share/" \
				|| die 
		fi
		rm -fr "${ED}/usr/lib" "${ED}/usr/share"

		sed -i -e "s:/usr/bin/python:${CPREFIX}/bin/python:g" \
			"${ED}${CPREFIX}/bin/pyuic4" || die ""
	fi
