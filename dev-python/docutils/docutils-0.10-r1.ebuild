...
DESCRIPTION_FR="Ensemble d'outils python pour transformer des documents en texte simple vers HTML, XML, etc..."
...
python_install_all() {
	...
	if use clip; then
		# create symlinks
		pushd "${ED}${cPREFIX:-usr}/bin/" > /dev/null
		for tool in *.py; do
			python_fix_shebang ${tool}
			ln -s ${tool} ${tool%%.*}
		done
		popd >/dev/null
	fi
