...
pkg_predeb() {
	if use clip; then
		# yes, we reuse chromium's libc for now
		patchelf --set-rpath "${CPREFIX:-/usr}/lib/chromium-browser:${CPREFIX:-/usr}/lib" \
			"${D}/${CPREFIX:-/usr}/bin/gdb" || die "Need dev-util/patchelf"

		pax-mark "-mepr" "${D}/${CPREFIX:-/usr}/bin/gdbserver" || die "unable to apply pax settings"
		pax-mark "-mepr" "${D}/${CPREFIX:-/usr}/bin/gdb" || die "unable to apply pax settings"
		pax-mark "-mepr" "${D}/${CPREFIX:-/usr}/bin/gdbreplay" || die "unable to apply pax settings"
	fi

	doverictld2 "${CPREFIX:-/usr}/bin/gdb" eu 'SYS_PTRACE' 'SYS_PTRACE' - cs
}
