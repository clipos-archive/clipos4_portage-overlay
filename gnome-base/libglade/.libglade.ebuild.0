...
inherit eutils gnome2 python virtualx deb
...
IUSE="doc static-libs test clip-deps"

RDEPEND="!clip-deps? ( >=dev-libs/glib-2.10:2 )
...
src_install() {
	dodir /etc/xml
	gnome2_src_install
	python_convert_shebangs 2 "${ED}${CPREFIX:-/usr}"/bin/libglade-convert
}
...
echo "* Updating XML catalog"
${CPREFIX:-/usr}/bin/xmlcatalog --noout --add "system" \\
	        "http://glade.gnome.org/glade-2.0.dtd" \\
			${CPREFIX:-/usr}/share/xml/libglade/glade-2.0.dtd ${CPREFIX}/etc/xml/catalog
EOF
	init_maintainer "prerm"
	cat >> "${ED}/DEBIAN/prerm" <<EOF
echo "* Updating XML catalog"
${CPREFIX:-/usr}/bin/xmlcatalog --noout --del \\
	        "http://glade.gnome.org/glade-2.0.dtd" \\
			${CPREFIX:-/usr}/share/xml/libglade/glade-2.0.dtd ${CPREFIX}/etc/xml/catalog
EOF
}
