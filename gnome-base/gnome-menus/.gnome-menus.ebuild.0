...
# $Header: /var/cvsroot/gentoo-x86/gnome-base/gnome-menus/gnome-menus-2.30.5.ebuild,v 1.2 2011/01/18 14:12:33 fauli Exp $
...
IUSE="debug +introspection python clip-deps clip"

RDEPEND="!clip-deps? ( >=dev-libs/glib-2.18 )
...
src_prepare() {
	...
	use clip && epatch "${FILESDIR}/${PN}-2.30-clip-filter-desktop-entries.patch"
...
src_install() {
	...
	mv "${ED}${CPREFIX}"/etc/xdg/menus/applications.menu \
		"${ED}${CPREFIX}"/etc/xdg/menus/gnome-applications.menu || die "menu move failed"

	exeinto "${CPREFIX}"/etc/X11/xinit/xinitrc.d/
