...
DEPEND="
	...
	!clip-deps? ( bzip2? ( app-arch/bzip2 ) )
...
RDEPEND="${DEPEND}
	...
	!clip? ( virtual/ssh )
	!clip? ( !aqua? ( !kernel_SunOS? ( virtual/eject ) ) )
"
...
src_install() {
	...
	if use minimal; then
		rm "${D}"/usr/lib64/libmolletnetwork.so "${D}"/usr/local/share/config.kcfg/jpegcreatorsettings.kcfg
		rmdir "${D}"/usr/local/share/config.kcfg/
	fi
}
