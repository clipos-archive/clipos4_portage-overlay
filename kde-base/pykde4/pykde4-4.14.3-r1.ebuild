...
IUSE="akonadi debug doc examples test clip clip-devstation"
...
pkg_setup() {
	...
	# Using the wrapper is not supported on CLIP (no eselect) and
	# does not work either on clip-devstation (I'm too lazy to investigate).
	use clip && have_python2=false
	use clip-devstation && have_python2=false
}
...
src_configure() {
	configuration() {
		...
		if use clip || use clip-devstation; then
			mycmakeargs+=(-DPYQT4_SIP_DIR=/usr/share/sip)
		fi
...
#	if use clip; then
#			mv "${ED}"${CPREFIX:-/usr}/bin/pykdeuic4{-${EPYTHON/python/},} || die
#		else
#			mv "${ED}"${CPREFIX:-/usr}/bin/pykdeuic4-{${EPYTHON/python/},${EPYTHON}} || die
#	fi
		mv "${ED%/}${CPREFIX:-/usr}/bin/pykdeuic4-${EPYTHON/python/}" \
	...
	use clip || dosym ../lib/python-exec/python-exec2 /usr/bin/pykdeuic4
