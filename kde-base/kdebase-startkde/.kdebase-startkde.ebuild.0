...
IUSE="+wallpapers clip clip-devstation"
...
RDEPEND="
	clip? (
			...
			$(add_kdebase_dep kdebase-workspace)
		  )
	clip-devstation? ( 
			$(add_kdeapps_dep kdebase-runtime-meta)
			$(add_kdebase_dep kdebase-workspace)
...
src_prepare() {
	...
	eprefixify "${T}/KDE-4"
	if use clip; then
		epatch "${FILESDIR}/clip-startkde4.patch"
			sed -e "s#@CPREFIX@#${CPREFIX}#" \
			-i startkde.cmake || die "Sed for REPLACE_PREFIX
			failed."
	else
...
src_install() {
	...
	doins "${FILESDIR}/agent-startup.sh"
	 sed -i -e "s#/usr/bin#${CPREFIX:-/usr}/bin#" \
	 		"${D}${CPREFIX}/etc/kde/startup/agent-startup.sh"

	insinto /etc/kde/shutdown
	doins "${FILESDIR}/agent-shutdown.sh"
	sed -i -e "s#/usr/bin#${CPREFIX:-/usr}/bin#" \
	        "${D}${CPREFIX}/etc/kde/startup/agent-shutdown.sh"
	...
	sed -e "s:\${BIN_INSTALL_DIR}:${EPREFIX}${CPREFIX:-/usr}/bin:g" \
