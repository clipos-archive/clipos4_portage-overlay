...
inherit kde4-base fdo-mime multilib toolchain-funcs flag-o-matic pax-utils verictl2
...
COMMONDEPEND="
	clip-devstation? ( clip-libs/clip-emblem )
	clip? ( 
		clip-libs/clip-emblem
		!<kde-base/kdelibs-4
		!<kde-base/kdebase-workspace-4.5
	)
	!clip-deps? (
		sys-libs/zlib
...
RDEPEND="${COMMONDEPEND}
	...
	!clip? ( $(add_kdebase_dep kde-env '' 4.14.3) )
	clip? (
		!<kde-base/kdebase-runtime-4.7.1
	)
	sys-apps/dbus[X]
	!aqua? (
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-4.12.5-webdavs.patch"
)
...
src_prepare() {
	...
	if use clip; then
		epatch "${FILESDIR}/${PN}-4.3.5-clip-bookmarks.patch"
		epatch "${FILESDIR}/${PN}-4.9.5-clip-cprefix.patch"
		epatch "${FILESDIR}/${PN}-4.12.5-emblem.patch"
	fi
}
...
src_install() {
	...
	rm -f "${ED}${PREFIX}"/share/apps/kssl/ca-bundle.crt || die
	...
	if use aqua; then
		...
		local _PV=${PV:0:3}.0
		local _dir=${EPREFIX}${KDEDIR}/$(get_libdir)/kde4/plugins/script
	...
	if use clip-devstation; then
		sed -i -e 's:/usr:${CMAKE_INSTALL_PREFIX}:g' \
			-e 's:/etc:${CMAKE_INSTALL_PREFIX}/etc:g' \
			"${ED}${KDEDIR}/share/apps/cmake/modules/KDELibsDependencies.cmake" \
			|| die
	fi

	if use clip; then
		pax-mark L "${ED}${KDEDIR}/bin/kdeinit4" || die "paxctl failed"
		rm -fr "${ED}${KDEDIR}/share/apps/cmake"
		# private smb.conf for ksambashare
		dodir "/etc/kde/samba"
		touch "${ED}${CPREFIX}/etc/kde/samba/smb.conf"
	fi

	einfo Installing environment file.
	# Since 44qt4 is sourced earlier QT_PLUGIN_PATH is defined.
	echo "COLON_SEPARATED=QT_PLUGIN_PATH" > "${T}/77kde"
	echo "QT_PLUGIN_PATH=${EPREFIX}${KDEDIR}/$(get_libdir)/kde4/plugins" >> "${T}/77kde"
	doenvd "${T}/77kde"
}
...
pkg_predeb() {
	kde4-base_pkg_predeb

	doverictld2 "${CPREFIX:-/usr}/bin/kdeinit4" e - - - c
	doverictld2 "${CPREFIX:-/usr}/bin/kded4" e - - - c
}
