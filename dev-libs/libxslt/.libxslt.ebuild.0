...
RDEPEND="
	...
	!core-deps? ( crypt?  ( >=dev-libs/libgcrypt-1.5.3:0=[${MULTILIB_USEDEP}] ) )
	python? (
		${PYTHON_DEPS}
...
multilib_src_configure() {
	libxslt_configure() {
		ECONF_SOURCE="${S}" econf \
			--with-html-dir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF} \
...
multilib_src_install() {
	...
	if multilib_is_native_abi && use python; then
		libxslt_foreach_py_emake \
			...
			EXAMPLE_DIR="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/python/examples \
			install
