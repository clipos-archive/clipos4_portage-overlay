...
static-libs clip-deps core-deps"
RESTRICT="test"

RDEPEND="!clip-deps? ( dev-libs/expat )
	...
	!core-deps? ( openssl? ( dev-libs/openssl ) )
...
src_configure() {
	...
	econf \
		--datadir=${CPREFIX:-/usr}/share/apr-util-1 \
		--with-apr=/usr \
		--with-expat=/usr \
		--without-sqlite2 \
		$(use_with freetds) \
...
src_install() {
	...
	find "${ED}${cPREFIX:-usr}/$(get_libdir)/apr-util-${SLOT}" -name "*.a" -exec rm -f {} +
	...
	rm -f "${ED}${cPREFIX:-usr}/$(get_libdir)/aprutil.exp"
}
