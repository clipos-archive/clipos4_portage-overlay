...
srp ssl static-libs urandom clip"

DEPEND="!clip? ( net-mail/mailbase )
...
multilib_src_configure() {
	...
	local statedir="/run/saslauthd"
	use clip && statedir="/var/shared/lib/sasl2"
	...
	econf \
		...
		--enable-otp \
		--without-sqlite \
		--with-saslauthd=${statedir} \
		--with-pwcheck=${statedir} \
		--with-configdir=${CPREFIX}/etc/sasl2 \
		--with-plugindir=${CPREFIX:-/usr}/$(get_libdir)/sasl2 \
		--with-dbpath=${CPREFIX}/etc/sasl2/sasldb2 \
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		...
		if use java ; then
			...
			java-pkg_regso "${D}${CPREFIX:-/usr}/$(get_libdir)/libjavasasl.so"
...
pkg_postinst () {
	...
	if ( use berkdb || use gdbm ) && [[ ! -f "${ROOT}/etc/sasl2/sasldb2" ]] ; then
		...
		# FIXME: CLIP SDK bootstrap: mail group does not exist at this point.
		# We should use a numeric value.
