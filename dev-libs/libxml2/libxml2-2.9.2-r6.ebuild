...
multilib_src_install() {
	emake DESTDIR="${D}" \
		EXAMPLES_DIR="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/examples install

	if multilib_is_native_abi && use python; then
		python_foreach_impl libxml2_py_emake \
			DESTDIR="${D}" \
			docsdir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/python \
			exampledir="${EPREFIX}${CPREFIX:-/usr}"/share/doc/${PF}/python/examples \
			install
...
multilib_src_install_all() {
	...
	if [[ ${CHOST} == *-winnt* ]]; then
		rm -rf "${ED}${CPREFIX:-/usr}"/bin/xmllint
		rm -rf "${ED}${CPREFIX:-/usr}"/bin/xmlcatalog
	fi

	rm -rf "${ED}${CPREFIX:-/usr}"/share/doc/${P}
	einstalldocs

	if ! use examples; then
		rm -rf "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/examples
		rm -rf "${ED}${CPREFIX:-/usr}"/share/doc/${PF}/python/examples
	fi
...
pkg_postinst() {
	...
	if [[ "${ROOT}" != "/" ]]; then
		...
		if [[ ! -e ${CATALOG} ]]; then
			...
			"${EPREFIX}${CPREFIX:-/usr}"/bin/xmlcatalog --create > "${CATALOG}"
...
pkg_predeb() {
	local CATALOGDIR="${CPREFIX}/etc/xml"
...
if [[ "\${1}" == "configure" || "\${1}" == "abort-remove" ]]; then
	if [[ ! -e "${CATALOG}" ]]; then
		[[ -d "${CATALOGDIR}" ]] || /bin/mkdir -p "${CATALOGDIR}"
		${CPREFIX:-/usr}/bin/xmlcatalog --create > "${CATALOG}"
		echo " * Created XML catalog in ${CATALOG}"
	fi
fi
EOF
}
