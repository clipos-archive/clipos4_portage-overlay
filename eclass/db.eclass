...
db_fix_so () {
	LIB="${ROOT}${CPREFIX:-/usr}/$(get_libdir)"
...
db_src_install_doc() {
	...
	if use doc; then
		...
		mv ${D}${CPREFIX:-/usr}/docs/* ${D}${CPREFIX:-/usr}/share/doc/${PF}/html/
		rm -rf ${D}${CPREFIX:-/usr}/docs
	else
		rm -rf ${D}${CPREFIX:-/usr}/docs
	fi
}
...
db_src_install_usrbinslot() {
	...
	for fname in ${D}${CPREFIX:-/usr}/bin/db_*
	do
...
db_src_install_headerslot() {
	...
	mv ${D}${CPREFIX:-/usr}/include/*.h ${D}${CPREFIX:-/usr}/include/db${SLOT}/
}

db_src_install_usrlibcleanup() {
	LIB="${D}${CPREFIX:-/usr}/$(get_libdir)"
	...
	rm -f ${D}${CPREFIX:-/usr}/include/db.h ${D}${CPREFIX:-/usr}/include/db_185.h ${LIB}/libdb.a ${LIB}/libdb_cxx.a
}
