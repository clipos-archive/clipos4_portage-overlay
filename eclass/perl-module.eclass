...
perl_delete_module_manpages() {
	...
	if [[ -d "${ED}${CPREFIX:-/usr}"/share/man ]] ; then
#		einfo "Cleaning out stray man files"
		find "${ED}${CPREFIX:-/usr}"/share/man -type f -name "*.3pm" -delete
		find "${ED}${CPREFIX:-/usr}"/share/man -depth -type d -empty -delete
	fi
}
...
perl_link_duallife_scripts() {
	...
	if has "${EBUILD_PHASE:-none}" "postinst" "postrm" ; then
		...
		pushd "${ED}" > /dev/null
		for i in $(find ${cPREFIX:-usr}/bin -maxdepth 1 -type f 2>/dev/null) ; do
			mv ${i}{,-${PV}-${P}} || die
			#DUALLIFESCRIPTS[${#DUALLIFESCRIPTS[*]}]=${i##*/}
			DUALLIFESCRIPTS[${#DUALLIFESCRIPTS[*]}]=${i}
		done
		for i in $(find ${cPREFIX:-usr}/share/man/man1 -maxdepth 1 -type f 2>/dev/null) ; do
			mv ${i} ${i%.1}-${PV}-${P}.1 || die
