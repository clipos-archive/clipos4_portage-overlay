...
case ${EAPI:-0} in
	0|1|2|3) die "Need to upgrade to at least EAPI=5" ;;
	4*|5*)   ;;
...
is_clip_gcc_lib() {
	# gcc-lib
	if [[ "${PN}" = "gcc" && "${DEB_NAME_SUFFIX}" =~ ^-lib.*$ ]]; then
		return 0
	fi
	return 1
}

if is_clip_gcc_lib; then
	PREFIX="${CPREFIX:-/usr}"
	BINPATH="${PREFIX}/bin"
	LIBPATH="${CLIP_LIBPATH:-${PREFIX}/$(get_libdir)}"
	DATAPATH="${PREFIX}/share"
	INCLUDEPATH="${PREFIX}/include"
	STDCXX_INCDIR="${INCLUDEPATH}"

	# Don't set to gcc-library or we'll get a compat libstdc++
	ETYPE="gcc-compiler"
fi

#---->> LICENSE+SLOT+I USE logic <<----
...
# before clip modification
# SLOT="${GCC_CONFIG_VER}"

#---->> DEPEND <<----

# CLIP: Open the !clip condition (for gcc-lib)
RDEPEND="!clip? (
	sys-libs/zlib
	nls? ( virtual/libintl )"
...
# CLIP: Close the !clip condition (for gcc-lib)
RDEPEND+=" )"
...
make_gcc_hard() {
	...
	# CLIP: force RELRO/BINDNOW
	gcc_hard_flags+="-DEFAULT_RELRO -DEFAULT_BIND_NOW"

	# CLIP: not only for use=hardened
	if true; then
		if hardened_gcc_works ; then
			einfo "Updating gcc to use automatic PIE + SSP building ..."
			gcc_hard_flags+=" -DEFAULT_PIE_SSP"
...
toolchain_src_compile() {
	...
	if is_clip_gcc_lib; then
		...
		use openmp && targets="${targets} all-target-libgomp"
		use gcj && targets="${targets} all-target-libjava"
		use libffi && targets="${targets} all-target-libffi"
		use objc && targets="${targets} all-target-libobjc"
...
toolchain_src_install() {
	...
	if is_clip_gcc_lib; then
		...
		use openmp && targets="${targets} install-target-libgomp"
		use gcj && targets="${targets} install-target-libjava"
		use libffi && targets="${targets} install-target-libffi"
		use objc && targets="${targets} install-target-objc"

		S="${WORKDIR}"/build emake -j1 DESTDIR="${D}" ${targets} || die

		# Move the libraries to the proper location
		[ -z "${CLIP_LIBPATH}" ] || gcc_movelibs
