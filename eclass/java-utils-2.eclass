...
java-pkg_dojar() {
	...
	local pref="${CPREFIX:-/usr}"
	local dest 
	if [[ "${JAVA_PKG_JARDEST##${pref}}" == "${JAVA_PKG_JARDEST}" ]]; then
		JAVA_PKG_JARDEST="$(echo "${JAVA_PKG_JARDEST}" | sed -e "s:^/usr:${pref}:")" 
	fi
...
java-pkg_doso() {
	...
	local pref="${CPREFIX:-/usr}"
	local dest 
	if [[ "${JAVA_PKG_LIBDEST##${pref}}" == "${JAVA_PKG_LIBDEST}" ]]; then
		JAVA_PKG_LIBDEST="$(echo "${JAVA_PKG_LIBDEST}" | sed -e "s:^/usr:${pref}:")" 
	fi
	# for each lib
	for lib in "$@" ; do
		# if the lib exists...
...
java-pkg_getjars() {
	...
	[[ -n "${CPREFIX}" ]] && \
		jars="$(echo "${jars}" | sed -e "s|/usr|${CPREFIX}|g")"
	echo "${jars}"
}
...
java-pkg_getjar() {
	...
	for jar in ${classpath//:/ }; do
		...
		if [[ "$(basename ${jar})" == "${target_jar}" ]] ; then
			...
			[[ -n "${CPREFIX}" ]] && \
				jar="$(echo "${jar}" | sed -e "s|/usr|${CPREFIX}|g")"
			echo "${jar}"
			return 0
		fi
	done
