...
gnome2_gconf_savelist() {
	...
	pushd "${ED}" &> /dev/null
	local dir="etc"
	[[ -n "${cPREFIX}" ]] && dir="${cPREFIX/etc}"
	export GNOME2_ECLASS_SCHEMAS=$(find "${dir}/gconf/schemas/" -name '*.schemas' 2> /dev/null)
	popd &> /dev/null
}
...
gnome2_schemas_savelist() {
	has ${EAPI:-0} 0 1 2 && ! use prefix && ED="${D}"
	pushd "${ED}" &>/dev/null
	export GNOME2_ECLASS_GLIB_SCHEMAS=$(find "${cPREFIX:-usr}/share/glib-2.0/schemas" -name '*.gschema.xml' 2>/dev/null)
	popd &>/dev/null
}
...
gnome2_gdk_pixbuf_savelist() {
	einfo "Pixbuf savelist"
	has ${EAPI:-0} 0 1 2 && ! use prefix && ED="${D}"
	pushd "${ED}" 1>/dev/null
	export GNOME2_ECLASS_GDK_PIXBUF_LOADERS=$(find "${cPREFIX:-usr}/$(get_libdir)/gdk-pixbuf-2.0" -type f 2>/dev/null)
	popd 1>/dev/null
}
