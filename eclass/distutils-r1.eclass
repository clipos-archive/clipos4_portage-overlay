...
_distutils-r1_wrap_scripts() {
	...
	if [[ -d ${path}${PYTHON_SCRIPTDIR} ]]; then
		if use clip; then # simply move all files to the actual install path
		...
		for f in "${python_files[@]}"; do
			...
			_python_ln_rel "${path}${EPREFIX}${CPREFIX:-/usr}"/lib/python-exec/python-exec2 \
...
distutils-r1_python_install() {
	...
	if [[ ! ${DISTUTILS_SINGLE_IMPL} ]]; then
		...
		local scriptdir=${EPREFIX}${CPREFIX:-/usr}/bin
	...
	esetup.py install --root="${root}" "${args[@]}" --prefix="${CPREFIX:-/usr}"

	if [[ -d ${root}$(python_get_sitedir)/tests ]]; then
		die "Package installs 'tests' package, file collisions likely."
	fi
	if [[ -d ${root}${CPREFIX:-/usr}/$(get_libdir)/pypy/share ]]; then
...
distutils-r1_python_install_all() {
	...
	if declare -p EXAMPLES &>/dev/null; then
		local INSDESTTREE=${CPREFIX:-/usr}/share/doc/${PF}/examples
		doins -r "${EXAMPLES[@]}"
