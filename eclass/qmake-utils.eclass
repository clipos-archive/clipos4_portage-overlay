...
qt4_get_libdir() {
	echo ${CPREFIX:-/usr}/$(get_libdir)/qt4
}
...
qt4_get_mkspecsdir() {
	echo ${CPREFIX:-/usr}/share/qt4/mkspecs
}
...
eqmake4() {
	...
	"$(qt4_get_bindir)"/qmake \
		-makefile \
		QTDIR="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir) \
		...
		QMAKE_LIBDIR_X11="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir) \
		QMAKE_LIBDIR_OPENGL="${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir) \
		"${qmake_args[@]}"
