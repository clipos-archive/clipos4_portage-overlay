...
C_TEXMF_PATH=${CPREFIX:-/usr}/share/texmf
C_TEXMF_DIST_PATH=${CPREFIX:-/usr}/share/texmf-dist
if use clip ; then
	C_TEXMF_VAR_PATH=/var/shared/lib/texmf
	TEXMF_VAR_PATH=/var/shared/lib/texmf
else
	C_TEXMF_VAR_PATH=/var/lib/texmf
...
texlive-common_handle_config_files() {
	...
	for f in $(find . -name '*.cnf' -type f -o -name '*.cfg' -type f | sed -e "s:\./::g") ; do
		...
		einfo "Moving (and symlinking) ${C_TEXMF_PATH}/${f} to ${CPREFIX}/etc/texmf/$(dirname ${f}).d"
		mv "${D}/${C_TEXMF_PATH}/${f}" "${D}${CPREFIX}/etc/texmf/$(dirname ${f}).d" || die "mv ${f} failed."
...
dobin_texmf_scripts() {
	while [ $# -gt 0 ] ; do
		...
		if [ "${PV#2008}" = "${PV}" -a "${PV#2009}" = "${PV}" ] ; then
			[ -x "${D}${CPREFIX:-/usr}/share/${1}" ] || die "Trying to install a non existing or non executable symlink to /usr/bin: ${1}"
		fi
...
etexmf-update() {
	if has_version 'app-text/texlive-core' ; then
		if [ "$ROOT" = "/" ] && [ -x ${CPREFIX:-/usr}/sbin/texmf-update ] ; then
...
efmtutil-sys() {
	if has_version 'app-text/texlive-core' ; then
		if [ "$ROOT" = "/" ] && [ -x ${CPREFIX:-/usr}/bin/fmtutil-sys ] ; then
			einfo "Rebuilding formats"
			${CPREFIX}:-/usr}/bin/fmtutil-sys --all &> /dev/null
		else
