...
inherit texlive-common base deb

HOMEPAGE="http://www.tug.org/texlive/"

COMMON_DEPEND=">=app-text/texlive-core-${TL_PV:-${PV}}"

IUSE="doc source clip"
...
### We already repackage upstream tarballs - 12/05/2010
SRC_URI="${SRC_URI} mirror://gentoo/${P}.tar.gz"
...
texlive-module_src_install() {
	...
	if [ -z "${PN##*documentation*}" ] || use doc; then
		[ -d texmf-doc ] && cp -pR texmf-doc "${D}${CPREFIX:-/usr}/share/"
	else
		[ -d texmf/doc ] && rm -rf texmf/doc
		[ -d texmf-dist/doc ] && rm -rf texmf-dist/doc
	fi

	[ -d texmf ] && cp -pR texmf "${D}${CPREFIX:-/usr}/share/"
	[ -d texmf-dist ] && cp -pR texmf-dist "${D}${CPREFIX:-/usr}/share/"
	[ -d tlpkg ] && use source && cp -pR tlpkg "${D}${CPREFIX:-/usr}/share/"

	if use clip; then
		insinto /var/shared/lib/texmf
	else
	...
	if [ -n "${TEXLIVE_MODULE_BINLINKS}" ] ; then
		for i in ${TEXLIVE_MODULE_BINLINKS} ; do
			[ -f "${ED}${CPREFIX:-/usr}/bin/${i%:*}" ] || die "Trying to install an invalid	BINLINK. This should not happen. Please file a bug."
			dosym ${i%:*} ${CPREFIX:-/usr}/bin/${i#*:} 
		done
	fi
...
# Generate postinst / prerm maintainer scripts to run texmf-update

texlive-module_pkg_predeb() {
	# texlive-* modules are updated a single time, when we update the texlive
	# metapackage.
	[[ "${PN##texlive-}" != "${PN}" ]] && return 0
	for s in "postinst" "prerm"; do
		init_maintainer "${s}"
		cat >>"${D}/DEBIAN/${s}" <<EOF
if [[ -x "${CPREFIX:-/usr}/sbin/texmf-update" ]]; then
	echo "* Running texmf-update *"
	${CPREFIX:-/usr}/sbin/texmf-update &>/dev/null || echo "! texmf-update failed !" >&2
fi
EOF
	done
}

# We already repackage tarballs (12/5/2010)
#if [ "${PV#2008}" != "${PV}" ]; then
