...
RDEPEND="
	!clip-deps? ( >=dev-libs/glib-2.6:2 )
	media-libs/gstreamer:${SLOT}
"

if [[ "${SLOT}" == "0.10" ]]; then
	# CLIP : avoid conflicting files - 0.10 ebuilds
	# will use another debian package name
	RDEPEND="${RDEPEND}
		clip? ( !<${CATEGORY}/${PN}-1.0 )"
fi
...
gst-plugins10_src_configure() {
	...
	local pname="Gentoo GStreamer ebuild"
	local porig="http://www.gentoo.org" 
	if use clip; then
		pname="Gstreamer"
		porig="Gstreamer"
	fi
