...
vim_src_prepare() {
	...
	echo '#define SYS_VIMRC_FILE "'${EPREFIX}${CPREFIX}'/etc/vim/vimrc"' >> "${S}"/src/feature.h
	echo '#define SYS_GVIMRC_FILE "'${EPREFIX}${CPREFIX}'/etc/vim/gvimrc"' >> "${S}"/src/feature.h
...
vim_src_install() {
	...
	local vimfiles="${CPREFIX:-/usr}"/share/vim/vim${VIM_VERSION/.}
	...
	if [[ ${PN} == "vim-core" ]] ; then
		...
		eprefixify "${ED}${CPREFIX}"/etc/vim/vimrc
		...
		if use livecd ; then
			...
			rm "${ED}${CPREFIX:-/usr}"/bin/vimtutor
		...
		eprefixify "${ED}${CPREFIX}"/etc/vim/gvimrc
	...
	use clip || rm -f "${ED}${CPREFIX:-/usr}"/share/man/man1/{ex,view}.1
}
