...
#	if [[ ${KV_MAJOR} -ge 3 ]]; then
	...
	local suf="${KERNEL_DISTFILES_COMPRESSION:-bz2}"
	...
	if [[ ${#OKV_ARRAY[@]} -ge 3 ]] && [[ ${KV_MAJOR} -ge 3 ]]; then
		...
		if [[ -z ${K_WANT_GENPATCHES} && -z ${K_GENPATCHES_VER} && ${KV_PATCH} -gt 0 ]]; then
			KERNEL_URI="${KERNEL_BASE_URI}/patch-${OKV}.${suf}"
			UNIPATCH_LIST_DEFAULT="${DISTDIR}/patch-${CKV}.${suf}"
		fi
		KERNEL_URI="${KERNEL_URI} ${KERNEL_BASE_URI}/linux-${KV_MAJOR}.${KV_MINOR}.tar.${suf}"
	else
		KERNEL_URI="${KERNEL_BASE_URI}/linux-${OKV}.tar.${suf}"
	fi
	...
	if [[ -n ${KV_EXTRA} ]]; then
		...
		KERNEL_URI="${KERNEL_BASE_URI}/patch-${CKV}.${suf}
					${KERNEL_BASE_URI}/linux-${OKV}.tar.${suf}"
		UNIPATCH_LIST_DEFAULT="${DISTDIR}/patch-${CKV}.${suf}"
	fi
...
if [[ ${ETYPE} == sources ]]; then
	...
	SLOT="0"
# CLIP : add "image" etype
elif [[ ${ETYPE} == image ]]; then
	DESCRIPTION="Kernel image for linux-${KV_MAJOR}.${KV_MINOR}"
	IUSE=""
	SLOT="0"
...
universal_unpack() {
	...
	local suf="${KERNEL_DISTFILES_COMPRESSION:-bz2}"
	if [[ ${#OKV_ARRAY[@]} -ge 3 ]] && [[ ${KV_MAJOR} -ge 3 ]]; then
		unpack linux-${KV_MAJOR}.${KV_MINOR}.tar.${suf}
	else
		unpack linux-${OKV}.tar.${suf}
	fi
