...
# CLIP: Directory prefix to use for everything
XDIR="${CPREFIX:-/usr}"
...
EXPORTED_FUNCTIONS="src_unpack src_compile src_install pkg_postinst pkg_postrm pkg_predeb"
case "${EAPI:-0}" in
...
if [[ ${FONT} == yes ]]; then
	...
	[[ -z ${FONT_OPTIONS} ]] && FONT_OPTIONS="--with-fontdir=\"${EPREFIX}${XDIR}/share/fonts/${FONT_DIR}\""
...
xorg-2_patch_source() {
	...
	if has clip-domains ${IUSE} && use clip-domains; then
		PATCHES+=( ${FILESDIR}/${P}-clip-domains.patch )
	fi
...
xorg-2_src_install() {
	...
	local install_args=( docdir="${EPREFIX}${XDIR}/share/doc/${PF}" )

	if [[ ${CATEGORY} == x11-proto ]]; then
		install_args+=(
			${PN/proto/}docdir="${EPREFIX}${XDIR}/share/doc/${PF}"
		)
	fi
...
remove_font_metadata() {
	...
	if [[ ${FONT_DIR} != Speedo && ${FONT_DIR} != CID ]]; then
		...
		rm -rf "${ED}"${XDIR}/share/fonts/${FONT_DIR}/fonts.{scale,dir,cache-1}
	fi
}
...
create_fonts_scale() {
	...
	if [[ ${FONT_DIR} != Speedo && ${FONT_DIR} != CID ]]; then
		ebegin "Generating fonts.scale"
			mkfontscale \
				-a "${EROOT}${XDIR}/share/fonts/encodings/encodings.dir" \
				-- "${EROOT}${XDIR}/share/fonts/${FONT_DIR}"
		eend $?
	fi
}
...
create_fonts_dir() {
	...
	ebegin "Generating fonts.dir"
			mkfontdir \
				-e "${EROOT}"${XDIR}/share/fonts/encodings \
				-e "${EROOT}"${XDIR}/share/fonts/encodings/large \
				-- "${EROOT}${XDIR}/share/fonts/${FONT_DIR}"
	eend $?
}
