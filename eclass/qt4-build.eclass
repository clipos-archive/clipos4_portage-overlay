...
if [[ "${CATEGORY}" == "dev-qt" ]]; then
	if [[ "${PN}" == "qt3support" ]]; then
		RDEPEND="clip? (
			>=virtual/qt-qt3support-4.8.4
		)"
	else
		RDEPEND="clip? (
			>=virtual/qt-${PN##qt}-4.8.4
		)"
	fi
fi
...
setqtenv() {
	...
	QTPREFIXDIR=${EPREFIX}${CPREFIX:-/usr}
	QTBINDIR=${QTPREFIXDIR}/bin
	HOST_QTBINDIR=${EPREFIX}/usr/bin
	...
	QTSYSCONFDIR=${EPREFIX}${CPREFIX}/etc/qt4
	...
	QT_MKSPECS_DIR="${QTDATADIR}/mkspecs"
	QMAKE_MKSPECS="${QTDATADIR}/mkspecs"
	unset QMAKESPEC
...
prepare_directories() {
	for x in "$@"; do
		...
		# avoid running over the maximum argument number, bug #299810
		{
			...
			-e "s:\$\$\[QT_INSTALL_DATA\]:${QTDATADIR}:g" \
			-e "s:\$\$\[QT_INSTALL_TRANSLATIONS\]:${QTTRANSDIR}:g" \
			-e "s:\$\$\[QT_INSTALL_IMPORTS\]:${QTIMPORTDIR}:g" \
			-e "s:\$\$\[QT_INSTALL_BINS\]:${QTBINDIR}:g" \
			-e "s:\$\$\[QT_INSTALL_PLUGINS\]:${QTPLUGINDIR}:g" \
			-e "s:\$\$\[QMAKE_MKSPECS\]:${QMAKE_MKSPECS}:g" \
			-e "s:\$\$\[QT_INSTALL_HEADERS\]:${QTHEADERDIR}:g" \
			|| die
		"${S}"/bin/qmake \
			"LIBS+=-L${QTLIBDIR}" \
			"CONFIG+=nostrip" \
...
symlink_binaries_to_buildtree() {
	for bin in qmake moc uic rcc; do
		ln -s "${HOST_QTBINDIR}"/${bin} "${S}"/bin/ || die "symlinking ${bin} to ${S}/bin failed"
	done
}
