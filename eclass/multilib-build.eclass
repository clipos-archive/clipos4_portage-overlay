...
multilib_check_headers() {
	_multilib_header_cksum() {
		# CLIP specific: handle CPREFIX
		[[ -d ${ED}${cPREFIX:-usr}/include ]] && \
		find "${ED}${cPREFIX:-usr}"/include -type f \
			-exec cksum {} + | sort -k2
	}
	...
	if [[ ${MULTILIB_WRAPPED_HEADERS[@]} ]]; then
		...
		if [[ ${MULTILIB_ABI_FLAG} ]]; then
			for f in "${MULTILIB_WRAPPED_HEADERS[@]}"; do
				...
				f=${f#${cPREFIX:-usr}/include}

				local dir=${f%/*}

				# Some ABIs may have install less files than others.
				if [[ -f ${root}{CPREFIX:-/usr}/include${f} ]]; then
...
_EOF_
					...
					mv "${root}${CPREFIX:-/usr}/include${f}" "${ED}/tmp/multilib-include/${CHOST}${dir}/" || die
...
multilib_install_wrappers() {
	...
	if [[ -d "${ED}"/tmp/multilib-include ]]; then
		multibuild_merge_root \
			"${ED}"/tmp/multilib-include "${root}${CPREFIX:-/usr}"/include
