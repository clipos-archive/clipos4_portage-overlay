...
if _python_implementation; then
	DEPEND="!clip? ( >=app-admin/eselect-python-20091230 )" 
	RDEPEND="${DEPEND}"
	PDEPEND="!clip? ( app-admin/python-updater )"
fi
...
_python_abi-specific_local_scope() {
	[[ " ${FUNCNAME[@]:2} " =~ " "(_python_final_sanity_checks|python_execute_function|python_mod_optimize|python_mod_cleanup|distutils_pkg_predeb|pkg_predeb)" " ]]
}
	...
	for file in "${files[@]}"; do
		...
		if [[ "$(head -n1 "${file}")" =~ ${_PYTHON_SHEBANG_BASE_PART_REGEX} ]]; then
			...
			if [[ -n "${CPREFIX}" && "${CPREFIX}" != "/usr" ]]; then
				sed -e "1s:^#!/usr/bin/python:#!${CPREFIX}/bin/python:" -i "${file}" \
					|| die "CPREFIX conversion of shebang in '${file}' failed"
			fi
		fi
	done
...
python_get_includedir() {
	...
	if [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "CPython" ]]; then
		echo "${prefix}${cPREFIX:-usr}/include/python${PYTHON_ABI}"
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "Jython" ]]; then
		echo "${prefix}${cPREFIX:-usr}/share/jython-${PYTHON_ABI%-jython}/Include"
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "PyPy" ]]; then
		echo "${prefix}${cPREFIX:-usr}/$(get_libdir)/pypy${PYTHON_ABI#*-pypy-}/include"
	fi
}
...
python_get_libdir() {
	...
	if [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "CPython" ]]; then
		echo "${prefix}${cPREFIX:-usr}/$(get_libdir)/python${PYTHON_ABI}"
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "Jython" ]]; then
		echo "${prefix}${cPREFIX:-usr}/share/jython-${PYTHON_ABI%-jython}/Lib"
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "PyPy" ]]; then
...
python_get_sitedir() {
	...
	if [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "CPython" ]]; then
		echo "${prefix}${cPREFIX:-usr}/$(get_libdir)/python${PYTHON_ABI}/site-packages"
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "Jython" ]]; then
		echo "${prefix}${cPREFIX:-usr}/share/jython-${PYTHON_ABI%-jython}/Lib/site-packages"
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "PyPy" ]]; then
		echo "${prefix}${cPREFIX:-usr}/$(get_libdir)/pypy${PYTHON_ABI#*-pypy-}/site-packages"
	fi
}
...
python_get_library() {
	...
	if [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "CPython" ]]; then
		if [[ "${linker_option}" == "1" ]]; then
			...
			echo "${prefix}${cPREFIX:-usr}/$(get_libdir)/libpython${PYTHON_ABI}$(get_libname)"
		fi
	elif [[ "$(_python_get_implementation "${PYTHON_ABI}")" == "Jython" ]]; then
...
_python_clean_compiled_modules() {
	...
	[[ "${FUNCNAME[1]}" =~ ^(python_mod_optimize|python_mod_cleanup|distutils_pkg_predeb|pkg_predeb)$ ]] || die "${FUNCNAME}(): Invalid usage"
