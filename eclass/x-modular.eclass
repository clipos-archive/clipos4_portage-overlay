...
inherit eutils libtool multilib toolchain-funcs flag-o-matic autotools \
	${FONT_ECLASS} ${GIT_ECLASS} deb

EXPORTED_FUNCTIONS="src_unpack src_compile src_install pkg_preinst pkg_postinst
pkg_postrm pkg_predeb"
...
x-modular_patch_source() {
	...
	# CLIP : apply clip-domains patch to those packages that need it
	if has clip-domains ${IUSE} && use clip-domains; then
		PATCHES="${PATCHES} ${FILESDIR}/${P}-clip-domains.patch"
	fi
...
x-modular_src_install() {
	...
	if [[ ${CATEGORY} = x11-proto ]]; then
		make \
			${PN/proto/}docdir=${XDIR}/share/doc/${PF} \
			DESTDIR="${D}" \
			install \
			|| die
	else
		make \
			docdir=${XDIR}/share/doc/${PF} \
			DESTDIR="${D}" \
			install \
			|| die
	fi
...
# Any documentation to install via dodoc
	...
	if [[ -e ${D}${XDIR}/$(get_libdir)/xorg/modules ]]; then
		find "${D}"${XDIR}/$(get_libdir)/xorg/modules -name '*.la' \
			| xargs rm -f
	fi
...
remove_font_metadata() {
	...
	for DIR in ${FONT_DIR}; do
		if [[ "${DIR}" != "Speedo" ]] && \
			...
			rm -f "${D}"${XDIR}/share/fonts/${DIR}/fonts.{scale,dir,cache-1}
		fi
	done
}
	...
	ebegin "Creating fonts.scale files"
		...
		for DIR in ${FONT_DIR}; do
			...
			if [[ "${x/encodings}" = "${x}" ]] \
				...
				mkfontscale \
					-a "${ROOT}"${XDIR}/share/fonts/encodings/encodings.dir \
					-- ${x}
			fi
		done
	eend 0
	...
	ebegin "Generating fonts.dir files"
		for DIR in ${FONT_DIR}; do
			...
			if [[ "${x/encodings}" = "${x}" ]]; then
				mkfontdir \
					-e "${ROOT}"${XDIR}/share/fonts/encodings \
					-e "${ROOT}"${XDIR}/share/fonts/encodings/large \
					-- ${x}
			fi
		done
	eend 0
