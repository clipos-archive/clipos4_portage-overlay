...
inherit kde4-functions toolchain-funcs fdo-mime flag-o-matic gnome2-utils virtualx versionator eutils multilib deb

IUSE+=" clip clip-deps clip-devstation"
...
EXPORT_FUNCTIONS pkg_setup src_unpack src_prepare ${export_fns} pkg_preinst pkg_postinst pkg_postrm pkg_predeb pkg_predeb
...
kdecommondepend="
	...
	clip? ( >=dev-qt/qtgui-4.8.5:4[accessibility,dbus(+)] )
	!clip? (  
		|| (
...
case ${KDE_REQUIRED} in
	always)
		...
		[[ -n ${kdedepend} ]] \
			&& DEPEND+=" ${kdedepend} clip? ( dev-qt/designer:4[-phonon] )"
	...
	optional)
		...
		[[ -n ${kdedepend} ]] \
			&& DEPEND+=" kde? ( ${kdedepend} ) clip? ( dev-qt/designer:4[-phonon] )"
...
kde4-base_pkg_setup() {
	...
	QT_PLUGIN_PATH="${EPREFIX}${CPREFIX:-/usr}/$(get_libdir)/kde4/plugins/"
...
kde4-base_src_configure() {
	...
	tc-is-cross-compiler && cmakeargs+=(-DKDE4_DATA_DIR=${ROOT}/usr/local/share/apps/)

	# sysconf needs to be /etc, not /usr/etc
	cmakeargs+=(-DSYSCONF_INSTALL_DIR="${EPREFIX}${CPREFIX}"/etc)
...
kde4-base_src_install() {
	...
	if use clip; then
		rm -fr "${ED}${PREFIX}/share/apps/cmake" 2>/dev/null
		rm -fr "${ED}${PREFIX}/lib/cmake" 2>/dev/null
	fi
...
# @FUNCTION: kde4-base_pkg_predeb
# @DESCRIPTION:
# Run before packaging a .deb
kde4-base_pkg_predeb() {
	debug-print-function ${FUNCNAME} "$@"

		buildsycoca # Only fixes permissions these days

# mimic postinst calls to fdo*update()
...
if [[ "\${1}" == "configure" || "\${1}" == "abort-remove" ]]; then
	...
	if [[ -x "${PREFIX}/bin/update-mime-database" ]]; then
		${PREFIX}/bin/update-mime-database "${PREFIX}/share/mime" \
				|| echo " !  Failed to update mime database !" >&2
	fi
fi
EOF
}
