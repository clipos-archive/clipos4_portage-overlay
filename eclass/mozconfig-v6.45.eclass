...
RDEPEND=">=app-text/hunspell-1.2:=
	...
	!clip? ( virtual/freedesktop-icon-theme )
	clip? (
		|| (
...
mozconfig_config() {
	...
	if has bindist ${IUSE}; then
		...
		if [[ ${PN} == firefox ]] && use bindist && use !clip ; then
			mozconfig_annotate '' --with-branding=browser/branding/aurora
		fi
		if [[ ${PN} == firefox ]] && use bindist && use clip ; then
			mozconfig_annotate '' --with-branding=browser/branding/official
		fi
	...
	mozconfig_annotate '' --prefix="${EPREFIX}${CPREFIX:-/usr}"
	mozconfig_annotate '' --libdir="${EPREFIX}${CPREFIX:-/usr}/"$(get_libdir)
...
mozconfig_install_prefs() {
	...
	echo "pref(\"spellchecker.dictionary_path\", \"${EPREFIX}${CPREFIX:-/usr}/share/myspell\");" \
		>>"${prefs_file}" || die
