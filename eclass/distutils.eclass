...
case "${EAPI:-0}" in
	0|1)
		EXPORT_FUNCTIONS src_unpack src_compile src_install pkg_postinst pkg_postrm pkg_predeb
		;;
	*)
		EXPORT_FUNCTIONS src_prepare src_compile src_install pkg_postinst pkg_postrm pkg_predeb
		;;
esac
...
distutils_src_install() {
	...
	if _python_package_supporting_installation_for_multiple_python_abis; then
		distutils_installation() {
			...
			for setup_file in "${DISTUTILS_SETUP_FILES[@]-setup.py}"; do
				...
				echo ${_BOLD}"$(PYTHON)" "${setup_file#*|}" "${_DISTUTILS_GLOBAL_OPTIONS[@]}" $([[ -z "${DISTUTILS_USE_SEPARATE_SOURCE_DIRECTORIES}" ]] && echo build -b "$(_distutils_get_build_dir)") install  --prefix="${CPREFIX:-/usr}" --no-compile --root="${T}/images/${PYTHON_ABI}" "$@"${_NORMAL}
				"$(PYTHON)" "${setup_file#*|}" "${_DISTUTILS_GLOBAL_OPTIONS[@]}" $([[ -z "${DISTUTILS_USE_SEPARATE_SOURCE_DIRECTORIES}" ]] && echo build -b "$(_distutils_get_build_dir)") install  --prefix="${CPREFIX:-/usr}" --no-compile --root="${T}/images/${PYTHON_ABI}" "$@" || return "$?"
		...
		for setup_file in "${DISTUTILS_SETUP_FILES[@]-setup.py}"; do
			...
			echo ${_BOLD}"$(PYTHON)" "${setup_file#*|}" "${_DISTUTILS_GLOBAL_OPTIONS[@]}" install --prefix="${CPREFIX:-/usr}" --root="${D}" --no-compile "$@"${_NORMAL}
			"$(PYTHON)" "${setup_file#*|}" "${_DISTUTILS_GLOBAL_OPTIONS[@]}" install --prefix="${CPREFIX:-/usr}" --root="${D}" --no-compile "$@" || die "Installation failed"
	...
	if [[ -z "${CPREFIX}" && -e "${ED}usr/local" ]]; then
...
distutils_pkg_postinst() {
	...
	if [[ -z "$(declare -p PYTHON_MODNAME 2> /dev/null)" ]]; then
		for pylibdir in "${EROOT}"${cPREFIX:-usr}$(get_libdir)/python* "${EROOT}"${cPREFIX:-usr}share/jython-*/Lib; do
...
distutils_pkg_postrm() {
	...
	if [[ -n "${PYTHON_MODNAME}" ]]; then
		if ! has "${EAPI:-0}" 0 1 2 || _python_package_supporting_installation_for_multiple_python_abis; then
			...
			for pymod in ${PYTHON_MODNAME}; do
				for pylibdir in "${EROOT}"${cPREFIX:-usr}/$(get_libdir)/python*; do
...
distutils_pkg_predeb() {
	for PYTHON_ABI in ${PYTHON_ABIS-${PYTHON_ABI:-$(PYTHON --ABI)}}; do
		local sitedir="$(python_get_sitedir)"
		if [[ -n "${CPREFIX}" ]]; then
			sitedir="$(echo "${sitedir}" | sed -e "s:/usr:${CPREFIX}:")"
		fi
		if [[ -e "${D}${sitedir}" ]]; then
			einfo "Optimizing python modules in ${sitedir}"
			"$(PYTHON)" $(python_get_libdir)/compileall.py "${D}${sitedir}" >/dev/null
			"$(PYTHON)" -O $(python_get_libdir)/compileall.py "${D}${sitedir}" >/dev/null
		fi
	done
}
