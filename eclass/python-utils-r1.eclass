...
python_export() {
	...
	for var; do
		case "${var}" in
			...
			PYTHON_SCRIPTDIR)
				local dir
				export PYTHON_SCRIPTDIR=${EPREFIX}${CPREFIX:-/usr}/lib/python-exec/${impl}
...
python_scriptinto() {
	...
	python_scriptroot="${CPREFIX:-/usr}${1##/usr}"
}
...
python_newexe() {
	...
	if use clip; then
		exeinto "${wrapd}"
		newexe "${f}" "${newfn}" || die
		python_fix_shebang "${ED}/${wrapd}/${newfn}"
		return $?
	fi #else
	(
		dodir "${wrapd}"
...
python_wrapper_setup() {
	...
	if [[ ! -x ${workdir}/bin/python ]]; then
		...
		if [[ ${EPYTHON} == python* ]]; then
			...
			# Python 2.7+.
			ln -s "${EPREFIX}${CPREFIX:-/usr}"/$(get_libdir)/pkgconfig/${EPYTHON/n/n-}.pc \
...
python_fix_shebang() {
	...
	for path; do
		...
		while IFS= read -r -d '' f; do
			...
			if [[ ! ${error} ]]; then
				if use clip; then
					sed -i -e "1s:^#!/usr/bin/${from}:#!${CPREFIX}/bin/python:" \
					"${f}" || die
				else
