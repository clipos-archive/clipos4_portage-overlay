...
src_prepare() {
	...
	if [[ -n "${CPREFIX}" && "${CPREFIX}" != "/usr" ]]; then
		for f in "${S}"/*.py "${S}/cupshelpers"/*.py; do
			sed -i -e "s:/usr/lib:${CPREFIX}/$(get_libdir):g" "${f}"
			sed -i -e "s:/usr/share:${CPREFIX}/share:g" "${f}"
			sed -i -e "s:/usr/bin/cups:${CPREFIX}/bin/cups:g" "${f}"
		done
	fi
}

src_configure() {
	local conf="--with-udev-rules"
	use clip && conf="--without-udev-rules"
...
src_compile() {
	...
	if [[ -n "${CPREFIX}" && "${CPREFIX}" != "/usr" ]]; then
			sed -i -e "s:/usr/libexec:${CPREFIX}/$(get_libdir):g" \
				build/lib/cupshelpers/config.py
	fi
}
