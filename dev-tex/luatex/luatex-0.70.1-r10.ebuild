...
# $Header: /var/cvsroot/gentoo-x86/dev-tex/luatex/luatex-0.70.1-r2.ebuild,v 1.11 2013/04/19 20:04:16 ago Exp $

EAPI=5

inherit libtool eutils deb
...
IUSE="doc clip-deps clip"

RDEPEND="dev-libs/zziplib
    >=media-libs/libpng-1.4
	clip? ( >=app-text/poppler-0.32.0:=[xpdf-headers(+)] )
	app-text/poppler:=[xpdf-headers(+)]
	!clip-deps? ( sys-libs/zlib )
	clip? ( app-text/texlive-core )
...
PATH=${CPREFIX:-/usr}/bin:\$PATH ${CPREFIX:-/usr}/bin/fmtutil-sys --all &>/dev/null || echo  " ! Failed to rebuild formats !" >&2
